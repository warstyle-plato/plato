...
<section class="sec" id="city"><h2>Что разрешает и требует город</h2><div id="cityBody">Загружаю решение и ТЭП…</div><h3 style="margin:16px 0 6px">Социальная инфраструктура по решению</h3><div id="socialBody" class="box">Ищу требования по ДОО, СОШ и медицине…</div><div class="links" id="sourceLinks"></div></section>
...
try{let so=(nag&&nag.social)||((await j('/krt-preview/proxy/'+encodeURIComponent(slug)+'/social')).objects||[]);if(so.length){$('socialBody').className='social-grid';$('socialBody').innerHTML=so.map(o=>'<div class="social-card"><b>'+esc(o.label||o.kind)+'</b><div>'+(o.places?fmt(o.places)+' мест':'места не указаны')+(o.area_sqm?' · '+fmt(o.area_sqm)+' м²':'')+'</div><small>'+esc(o.quote||'')+'</small></div>').join('')}else{$('socialBody').className='box';$('socialBody').textContent='В прочитанном проекте решения отдельные требования по ДОО, СОШ или поликлинике не найдены.'}}catch(e){$('socialBody').className='box';$('socialBody').textContent='Социальные требования не прочитаны: '+e.message}
...
def install(app):
    ...
    async def krt_preview_nagatino_source():
        ...
        try:
            ...
            from market_search.krt_requirements import parse_decision_requirements, social_objects_from_decision
            req_facts = parse_decision_requirements(text)
            social = social_objects_from_decision(req_facts.get("construction") or [])
