...
function siteMap(site,row){
 const box=$('topMap'),rings=(site&&site.rings_merc)||[];
 if(!rings.length){box.innerHTML='<div class="box">Контур территории на карте пока не получен.</div>';return}
 const pts=rings.flat().filter(p=>Array.isArray(p)&&p.length>=2);
 if(!pts.length){box.innerHTML='<div class="box">Контур территории пуст.</div>';return}
 const xs=pts.map(p=>Number(p[0])),ys=pts.map(p=>Number(p[1])),minx=Math.min(...xs),maxx=Math.max(...xs),miny=Math.min(...ys),maxy=Math.max(...ys),dx=Math.max(300,maxx-minx),dy=Math.max(300,maxy-miny),pad=Math.max(dx,dy)*.32,ax=minx-pad,bx=maxx+pad,ay=miny-pad,by=maxy+pad,w=1000,h=560,px=x=>(x-ax)/(bx-ax)*w,py=y=>h-(y-ay)/(by-ay)*h;
 const paths=rings.map(r=>'M'+r.map(p=>px(Number(p[0])).toFixed(1)+' '+py(Number(p[1])).toFixed(1)).join('L')+'Z').join(' ');
 const src='https://plato-development-investment-model.onrender.com/land/basemap?'+new URLSearchParams({bbox:[ax,ay,bx,by].join(','),width:String(w)});
 box.innerHTML='<div class="mapstage"><img src="'+src+'" alt="Карта территории КРТ"><svg viewBox="0 0 '+w+' '+h+'" preserveAspectRatio="none"><path d="'+paths+'" fill="#111" fill-opacity=".18" stroke="#111" stroke-width="5" vector-effect="non-scaling-stroke"></path></svg><div class="mapnote">'+esc((row.name||'КРТ')+(site.outline_source==='decision'?' · контур по перечню решения':' · официальный контур реестра'))+'</div></div>';
}
...
$('cityBody').innerHTML='<div class="cols"><div class="box"><b>ТЭП из проекта решения'+(dm.id?' №'+esc(dm.id):'')+'</b><br>Площадь территории '+fmt(tep.area_ha,2)+' га<br>Предельная СПП '+fmt(tep.total_gfa_sqm)+' м²<br>Жилое назначение '+fmt(tep.housing_gfa_sqm)+' м²<br>Нежилое назначение '+fmt(tep.nonresidential_gfa_sqm)+' м²'+(tep.flats_sqm!=null?'<br>Площадь квартир '+fmt(tep.flats_sqm)+' м²':'')+'</div><div class="box"><b>Карточка каталога — для сверки</b><br>СПП '+fmt(r.total_gfa_sqm)+' м²<br>Жильё '+fmt(r.housing_gfa_sqm)+' м²<br>Нежилое '+fmt(r.nonresidential_gfa_sqm)+' м²<br>Статус '+esc(r.status||'—')+'</div></div>';if(nag&&nag.city_requirements&&nag.city_requirements.nonhousing_obligations_sqm){let c=Number(nag.city_requirements.nonhousing_obligations_sqm),d=Number(tep.nonresidential_gfa_sqm);$('cityBody').innerHTML+='<div class="box"><b>Проверка разложения нежилой части</b><br>Обязательства по п. 4.1 дают '+fmt(c)+' м²'+(Number.isFinite(d)?' из '+fmt(d)+' м² нежилой СПП решения · '+(Math.abs(c-d)<=1?'сходится':'НЕ СХОДИТСЯ'):'')+'.</div>'}
try{
 let cr=nag&&nag.city_requirements;
 if(cr&&cr.objects&&cr.objects.length){
   $('socialBody').className='';
   let cards=cr.objects.map(o=>{
     let bits=[];
     if(o.places)bits.push(fmt(o.places)+' мест');
     if(o.area_sqm)bits.push((/business/.test(o.kind)?'общая площадь ':'площадь не менее ')+fmt(o.area_sqm)+' м²');
     if(o.land_area_ha)bits.push('участок не менее '+fmt(o.land_area_ha,2)+' га');
     if(o.transfer_to_city)bits.push('передать в собственность Москвы');
     return '<div class="social-card"><b>'+esc(o.clause+' · '+o.label)+'</b><div>'+esc(bits.join(' · '))+'</div><small>'+esc(o.quote||'')+'</small></div>'
   }).join('');
   let extra=[];
   if(cr.nonhousing_obligations_sqm)extra.push('<div class="box"><b>Расшифровка нежилой СПП</b><br>Сумма обязательных объектов: '+fmt(cr.nonhousing_obligations_sqm)+' м². Для Нагатино это должно сходиться с нежилой СПП решения.</div>');
   if(cr.infrastructure_agreement)extra.push('<div class="box"><b>4.3 · Инфраструктурное соглашение</b><br>'+esc(cr.infrastructure_agreement)+'</div>');
   if(cr.implementation_years)extra.push('<div class="box"><b>Срок реализации</b><br>'+fmt(cr.implementation_years)+' лет</div>');
   if(cr.planning_months)extra.push('<div class="box"><b>Срок подготовки ДПТ</b><br>'+fmt(cr.planning_months)+' месяцев</div>');
   if(cr.parameters_adjustable)extra.push('<details><summary>Оговорка об уточнении параметров</summary><div class="box">'+esc(cr.parameters_adjustable)+'</div></details>');
   if(cr.operator_exception)extra.push('<details><summary>Исключение для юрлица, определённого городом</summary><div class="box">'+esc(cr.operator_exception)+'</div></details>');
   $('socialBody').innerHTML='<div class="social-grid">'+cards+'</div>'+extra.join('');
 }else{
   let so=((await j('/krt-preview/proxy/'+encodeURIComponent(slug)+'/social')).objects||[]);
   if(so.length){$('socialBody').className='social-grid';$('socialBody').innerHTML=so.map(o=>'<div class="social-card"><b>'+esc(o.label||o.kind)+'</b><div>'+(o.places?fmt(o.places)+' мест':'места не указаны')+(o.area_sqm?' · '+fmt(o.area_sqm)+' м²':'')+'</div><small>'+esc(o.quote||'')+'</small></div>').join('')}
   else{$('socialBody').className='box';$('socialBody').textContent='В прочитанном проекте решения отдельные требования по социальным объектам не найдены.'}
 }
}catch(e){$('socialBody').className='box';$('socialBody').textContent='Требования города не прочитаны: '+e.message}
