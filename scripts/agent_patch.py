...
# Excel files are accepted directly in Telegram. Strict DevelopAid template,
# GlavAPU calculator and generic TEP workbooks all feed the same guided flow.
...
def _telegram_dialog_data_from_parsed_excel(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    mapping = {
        "apartments_saleable_sqm": product_value("apartments", "saleable"),
        "apartments_gns_sqm": product_value("apartments", "gns"),
        "commercial_saleable_sqm": product_value("ground_commercial", "saleable"),
        "commercial_gns_sqm": product_value("ground_commercial", "gns"),
        "parking_spaces": product_value("underground_parking", "units"),
        "offices_gba_sqm": product_value("offices", "gns") or float(inputs.get("offices_gba_sqm") or 0),
        "offices_saleable_sqm": product_value("offices", "saleable") or float(inputs.get("offices_saleable_sqm") or 0),
        "retail_gba_sqm": product_value("standalone_retail", "gns") or float(inputs.get("retail_gba_sqm") or 0),
        "retail_saleable_sqm": product_value("standalone_retail", "saleable") or float(inputs.get("retail_saleable_sqm") or 0),
        "above_parking_spaces": product_value("above_parking", "units") or float(inputs.get("above_parking_spaces") or 0),
        "above_parking_gns_sqm": product_value("above_parking", "gns"),
        "kindergarten_places": product_value("kindergarten", "units") or float(inputs.get("kindergarten_places") or 0),
        "school_places": product_value("school", "units") or float(inputs.get("school_places") or 0),
        "clinic_capacity": product_value("clinic", "units") or float(inputs.get("clinic_capacity") or 0),
    }
    for key, value in mapping.items():
        if value > 0:
            data[key] = int(round(value)) if key.endswith("_places") or key in {"parking_spaces", "above_parking_spaces"} else value

    # Monetary fields: zero from a blank Excel cell means "not provided", not a confirmed zero.
    ...
    # GlavAPU normalized metadata may contain district.
...
def _telegram_parse_generic_tep_xlsx(data: bytes, filename: str) -> dict[str, Any]:
    ...
    for sheet, rows in tables.items():
        ...
        for row in rows[:400]:
            ...
            if sum(len(x) for x in chunks) > 55000:
                break
        if sum(len(x) for x in chunks) > 55000:
            break
...
def _telegram_handle_manual_document(chat_id: int, document: dict[str, Any]) -> None:
    ...
    # 3) Arbitrary TEP workbook: flatten tables and recognize labels/values.
    if parsed is None:
        try:
