...
def _build_developaid_pdf(payload: dict[str, Any]) -> bytes:
    ...
    story.append(PageBreak());story.append(P("Структура расходов",h2))
    ...
    finance_rows=[["Показатель","Значение"],["Расчётный БРИДЖ",_pdf_money(financing.get('calculated_bridge'))],["Пиковый фактический БРИДЖ",_pdf_money(financing.get('actual_bridge'))],["Пиковая (непокрытая эскроу) задолженность ПФ",_pdf_money(financing.get('pf_uncovered_peak'))],["Лимит ПФ",_pdf_money(financing.get('pf_limit'))],["Средняя ставка БРИДЖ",_pdf_pct(financing.get('avg_bridge_rate'))],["Средняя фактическая ставка ПФ",_pdf_pct(financing.get('avg_pf_effective_rate'))],["Проценты и комиссии",_pdf_money(financing.get('interest_and_fees'))],["LLCR",_pdf_num(summary.get('llcr'),2)+"x"]]
    story.append(table(finance_rows,[112*mm,58*mm]))
    events=calendar_data.get('events') or []
    if events:
        story.append(P("Календарь проекта",h2));event_rows=[["Этап","Начало","Окончание","Группа"]]
    ...
    if notes:
        ...
        for value in notes.values():
            if value: story.extend([P("• "+str(value),small),Spacer(1,1.5*mm)])
...
'''
if anchor not in s: raise SystemExit('telegram template anchor not found')
helper=r'''
...
new='''    _telegram_send_message(chat_id, text, reply_markup=button)
    ...
    return {"ok": True}
'''
if old not in s: raise SystemExit('telegram result send anchor not found')
s=s.replace(old,new,1)

new_func=r'''function currentPdfReportPayload(cads=[]){
...
'''
pattern=r'async function exportReportPdf\(\)\{.*?\n\}\n(?=function persistLocalSilently)'
s,count=re.subn(pattern,lambda _m:new_func,s,count=1,flags=re.S)
if count!=1: raise SystemExit(f'exportReportPdf replacement count={count}')
...
'''
if old_payload not in s: raise SystemExit('telegram payload end anchor not found')
...
for marker in ('version="0.12.23"','def _build_developaid_pdf','@app.post("/report/pdf")','def _telegram_send_document_bytes','report_payload:currentPdfReportPayload(cads)','async function exportReportPdf()',"fetch('/report/pdf'"):
