...
# 1) Calculate the metric the user actually means: maximum PF principal not covered by escrow.
...
# 2) Keep the same metric correct for consolidated/phased calculations.
old = '''    peak_pf = max((r["pf_balance"] for r in rows), default=0.0)
...
# 3) Expose both gross peak PF (for internal/report compatibility) and uncovered PF in report financing.
...
# 4) Telegram preliminary economics: add all-in expenses and use the uncovered PF metric.
...
# 5) Send the new metrics from the calculated result to Telegram.
...
# User-facing model report: do not call gross PF balance "uncovered". Where the old KPI label exists,
# switch the value to the newly calculated uncovered metric at the same time.
s = s.replace("['Пиковый ПФ',f.pf_peak]", "['Макс. не покрытая эскроу задолженность по ПФ',f.pf_uncovered_peak]")
s = s.replace("['Пиковый ПФ', f.pf_peak]", "['Макс. не покрытая эскроу задолженность по ПФ', f.pf_uncovered_peak]")
s = s.replace('Пиковый ПФ', 'Макс. не покрытая эскроу задолженность по ПФ')
s = s.replace('пиковый ПФ', 'макс. не покрытая эскроу задолженность по ПФ')
