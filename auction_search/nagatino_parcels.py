...
def land_holdings(view: dict[str, Any] | None = None,
    ...
    1. **Земля.** Собственник по ЕГРН; записи нет, но участок сдан городом в
       аренду (договор «М-05-…» или «…-05 ДГИ») — Москва как документально
       установленный арендодатель. Если договора и зарегистрированного
       собственника нет, правообладатель НЕ установлен: возможный режим
       неразграниченной государственной собственности остаётся отдельным
       аналитическим выводом и не превращается в собственника «Москва».
    ...
    for number, land in lands.items():
        ...
        if contract:
            ...
            # Общее правило о распоряжении неразграниченной землёй не является
            # записью о собственнике. Аналитическая гипотеза не должна
            # превращаться в правообладателя «Москва».
            inferred = str(disposal.get("ground") or "").strip()
            holder_of[number] = {
                "name": "",
                "note": "правообладатель не установлен по ЕГРН",
                "group": None,
                "by": ("нет зарегистрированного собственника"
                       + (f"; {inferred}" if inferred else "")),
                "group_title": "Правообладатель не установлен по ЕГРН",
