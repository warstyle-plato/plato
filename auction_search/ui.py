...
#krtTableWrap table.wide th:nth-child(1),#krtTableWrap table.wide td:nth-child(1){width:31%}
#krtTableWrap table.wide th:nth-child(2),#krtTableWrap table.wide td:nth-child(2){width:11%}
#krtTableWrap table.wide th:nth-child(3),#krtTableWrap table.wide td:nth-child(3){width:12%}
...
<th data-sort="score" title="Инвестиционная оценка площадки">Оценка</th>
...
function renderKrt(){const a=state.krtFiltered,body=$('krtRows');body.innerHTML='';renderKrtFilterNote();$('krtEmpty').style.display=a.length?'none':'grid';$('krtCount').textContent=a.length;
 ...
 tr.innerHTML=`<td><div class="lotname"><span>${esc(x.name)}</span><div class="krt-tags">${fresh}${krtNameFlags(x)}${tender}${nocard}${marks}</div></div><div class="source">${esc(krtBroken(x)?'разбор карточки съехал на поле — округ и ТЭП не показаны':[x.okrug,x.district].filter(Boolean).join(' · '))}</div></td><td><span class="fit ${sc.tone}" title="${esc(title)}"><span class="light"></span>${krtScoreNumber(sc)} · ${esc(sc.label)}</span><div class="source">${esc(krtScoreNote(sc))}</div></td><td class="money">${krtRankCell(x.slug)}</td><td class="money">${krtModelCell(x.slug,'llcr')}</td><td class="money">${krtModelCell(x.slug,'margin')}</td><td class="money">${krtMarketPriceCell(x.slug)}</td><td>${krtBroken(x)?krtUnparsedCell(x):(x.area_ha?esc(x.area_ha+' га'):'—')}</td><td>${krtBroken(x)?krtUnparsedCell(x):fmtArea(x.total_gfa_sqm)}</td><td>${krtBroken(x)?krtUnparsedCell(x):fmtArea(x.housing_gfa_sqm)}</td>`;tr.onclick=()=>selectKrt(x);body.appendChild(tr)});renderAskContext();syncTopScroll()}
