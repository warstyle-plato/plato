...
# Номер в НАЧАЛЕ клетки, а не во всей клетке. Прежний гейт требовал
# `_CAD.fullmatch` по всему значению под подписью — и печатная форма, у которой
# под подписью стоит номер и приклеенный к нему соседний текст, отказывала
# целиком. Замер прода 16.09.2026 по девяти складам лотов: отказов печатной
# формы на последнем заходе читателя 55 — 51 на 1-й Горловской, по два на обоих
# Прожекторах, — и у ВСЕХ причина одна: «под подписью форма
# «99:99:9999999:9999 бббббббббб»» и её продолжения. То есть клетка начинается
# полным кадастровым номером, а дальше идёт слово.
#
# Что это за слово, опознавать не нужно и угадывать нечего: буква частью
# кадастрового номера не бывает, поэтому ведущее совпадение читается безопасно,
# ЧТО БЫ за ним ни стояло. Взгляд вперёд нужен затем, чтобы совпадение не
# кончилось внутри последней группы: без него «:2054» прочиталось бы как «:2» —
# то же, что «3 306 021 ₽/м² → 306 021».
_CAD_HEAD = re.compile(r"^\s*(\d+:\d+:\d+:\d+)(?![\d:])")
# Квартал — три группы, не четыре. Он читается тем же правилом потому, что
# правка выше превращает отказ в ЗАПИСЬ: клетка квартала на тех же документах
# склеивается так же, и прежде это уезжало в отказ, а теперь уехало бы в поле
# молча — то есть на месте отказа встало бы неверное значение, что хуже
# отказа. Не опознали форму — оставляем клетку как есть: это прежнее
# поведение, а не новая потеря.
_QUARTER_HEAD = re.compile(r"^\s*(\d+:\d+:\d+)(?![\d:])")


def _ambiguous(tail: str) -> bool:
    """Стоит ли в хвосте ВТОРОЙ номер — тогда ведущий читать нельзя.

    Замер прода 16.09.2026 по девяти складам: из 55 отказов печатной формы за
    номером во ВСЕХ 55 идут буквы («99:99:9999999:9999 бббббббббб» и её
    продолжения), второго номера нет ни в одном. Буква частью кадастрового
    номера не бывает, поэтому такой хвост ведущему номеру не мешает.

    А вот «77:05:0012007:2054 21.06.2024 77:05:0012007:2055» — другой случай:
    в клетке ДВА номера, и какой из них номер объекта, документ не говорит.
    Прочитать первый молча значило бы поставить на место отказа неверное
    значение — то же, что обрезка, прячущая непонятое. Такая клетка остаётся
    отказом, а её форма называется в диагнозе.
    """
    return bool(re.search(r"\d+:\d+:\d+", tail or ""))


def _leading(pattern: re.Pattern[str], cell: str) -> tuple[str, str]:
    """Номер в начале клетки и приклеенный к нему хвост, порознь.

    Хвост возвращается затем, чтобы его не выбросить молча: молча выброшенное
    читается как его отсутствие. Наружу он идёт ФОРМОЙ, а не значением, — по
    той же причине, по какой формой говорит отказ: в выписке стоят имена
    правообладателей, а запись уезжает в свод площадки и в чат.
    """
    match = pattern.match(cell or "")
    if not match:
        return "", ""
    return match.group(1), (cell or "")[match.end():].strip()
...
def read_text(text: str, *, text_source: str = "layer") -> dict[str, Any]:
    ...
    found = _values(lines)
    # Клетка номера читается с НАЧАЛА: под подписью стоит номер, а за ним
    # бывает приклеена соседняя строка (замер прода 16.09.2026: 55 файлов
    # трёх лотов). Отказ остаётся там, где ведущего номера нет вовсе.
    cell = _first(found, "cadastral_number")
    number, glued = _leading(_CAD_HEAD, cell)
    if number and _ambiguous(glued):
        number, glued = "", glued
    if not number:
        raise ValueError(_why_no_number(found, text, kind, text_source))
    quarter_cell = _first(found, "quarter")
    quarter, quarter_glued = _leading(_QUARTER_HEAD, quarter_cell)
    if quarter and _ambiguous(quarter_glued):
        # Квартал тем же правилом: два номера в клетке — значит непонятое, и
        # молча взятый первый хуже пустого поля.
        quarter, quarter_glued = "", quarter_glued
    # Склейки названы формой и своим полем: это наш артефакт разбора, а не
    # ответ документа, и потому он не стоит рядом со значениями выписки.
    glued_cells = {key: _shape(tail) for key, tail in
                   (("cadastral_number", glued), ("quarter", quarter_glued))
                   if tail}
    ...
    record: dict[str, Any] = {
        ...
        "quarter": quarter or quarter_cell,
    ...
    if glued_cells:
        record["glued_cells"] = glued_cells
    if kind == "land":
        record.update({
            "area_kind": "",
