...
def looks_like_pdf(data: bytes) -> bool:
    """PDF опознаётся первыми байтами — по той же причине, что и зип.

    Расширения у записи архива может не быть вовсе, а подпись площадки врёт в
    обе стороны. Печатная форма выписки без расширения иначе попадала бы в
    непрочитанное с диагнозом «не XML выписки» — то есть наш пробел выглядел бы
    как негодный файл.
    """
