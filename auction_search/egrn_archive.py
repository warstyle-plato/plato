...
# 5 — предел длины отказа 200 → 400: диагноз печатной формы в 288 знаков
#     ложился на склад обрезанным, и решающий конец («форма клетки номера»)
#     терялся молча. Версия поднята по тому же правилу, что и четвёртая, и
#     поднять её здесь обязательно: без этого 56 отказов, записанных обрезанными
#     (замер прода 15.09.2026), считались бы свежими — `stale()` смотрит на
#     версию, а не на длину, — и починка до них не доехала бы НИКОГДА, то есть
#     ровно до того, ради чего она написана.
# 6 — кадастровый номер печатной формы берётся из НАЧАЛА клетки: под
#     подписью стоит номер, а за ним бывает приклеена соседняя строка, и
#     прежний `fullmatch` отказывал целиком. Замер прода 16.09.2026: так
#     молчали 55 файлов трёх лотов, и у всех отказ называл одну причину.
#     Версия поднята по тому же правилу, что четвёртая и пятая: ОТВЕТ
#     читателя на те же байты изменился, а без подъёма эти 55 отказов
#     считались бы свежими и починка до них не доехала бы никогда.
READER_VERSION = 6
