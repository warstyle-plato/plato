...
FUNCTIONS = (
    ...
    "landNum", "escapeHtml",
    # Площадь колец. Заполненный контур перехватывает указатель на всей своей
    # площади, поэтому порядок отрисовки задаётся ею: крупные вниз, мелкие
    # наверх — иначе мелкий недостижим в принципе. Считать её второй формулой
    # незачем: у движка она есть.
    "landRingArea",
)
