...
class RoseltorgAdapter(AuctionPlatformAdapter):
    ...
    def fetch_lot(self, lot_url: str, *, deadline: float | None = None) -> AuctionLot:
        ...
        for field, value in {
            ...
            "land_area_sqm": str(area) if area is not None and lot_kind in {LotKind.LAND_SALE, LotKind.LAND_LEASE, LotKind.KRT} else None,
            "building_area_sqm": str(area) if area is not None and lot_kind in {LotKind.PROPERTY_COMPLEX, LotKind.UNFINISHED} else None,
            "seller": seller,
            "organizer": organizer,
            "procedure_type": procedure_method,
