...
Раздел разведка читает — с 0.21.x, и с 0.21.65 постранично: до первой
страницы без новых ссылок, не больше `SECTION_MAX_PAGES`. Поиск по тегам
(`/procedures/search?tags[]=…`, `RoseltorgAdapter.DISCOVERY_TAGS`) остаётся
вторым входом. Своего разбора карточек раздела здесь по-прежнему нет: проба
показывает ответ, а код пишется по нему, а не по догадке.

Прежде тут стояло «наш читатель этот раздел не спрашивает вовсе» — верное
летом и неверное с осени. Оговорка обязана переживать смену источника: устав
быть правдой, она продолжает читаться как правда.
...
SECTIONS: tuple[tuple[str, str], ...] = (
    ...
    # Страницы раздела: на экране владельца в нём 44 процедуры (02.09.2026), а
    # разведка спрашивала одну страницу. Сколько даёт вторая и третья — это и
    # есть ответ на «почему в торгах один КРТ».
    ("Тот же адрес, страница 2 — есть ли продолжение",
     RoseltorgAdapter._paged(CATALOGUE_URL, 2)),
    ("Тот же адрес, страница 3",
     RoseltorgAdapter._paged(CATALOGUE_URL, 3)),
...
def probe(seconds: float = 45.0, directory: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        # 0.21.x, и текст «не спрашиваем вовсе» стал неверным, а остался.
        "parsing": ("разбора нет: сначала ответ источника, потом код. "
                    "Раздел «Развитие территории» разведка читает постранично "
                    f"(до {RoseltorgAdapter.SECTION_MAX_PAGES} страниц), а поиск "
                    "по тегам остаётся вторым входом."),
        "today_discovery": {
