...
async function load(refresh){
 ...
 try{
  ...
  $('gate').style.display='';
  // Отказ называет ОБА ключа поимённо. Прежний говорил «задайте ключ
  // администратора», и человек с настоящим ключом кабинета рынка читал это
  // как «ключ не тот», хотя ключ был верный — просто другой: у сервиса их два,
  // и на экране они неразличимы.
  $('gate').innerHTML='<b>Данные не показаны:</b> '+escapeHtml(e.message)
   +'<div style="margin-top:6px">Открыть страницу может любой из трёх: '
   +'вход через Telegram в «Личном кабинете»; ключ администратора сервиса '
   +'(<code>DEVELOPAID_ADMIN_KEY</code>, он же открывает готовые примеры); '
   +'ключ кабинета рынка (<code>MARKET_CABINET_KEY</code> — тот, которым '
   +'открывается <a href="/cabinet">/cabinet</a>). Ключи РАЗНЫЕ: кабинетный '
   +'живёт кукой и в «Личном кабинете» его не задают. Проще всего — попросить '
   +'у владельца ссылку «Поделиться»: она открывает страницу без всяких ключей.</div>';
  return;
 }
 render();
 poll();
