...
def _programme(
    ...
    # Названную решением площадь помечаем требованием: без признака движок
    # пересчитает её нормативом, и число города потеряется молча — «приоритет
    # по полю» решает один ответ на всех писателей вводных.
