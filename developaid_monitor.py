...
def _project_dir(project: str) -> Path:
    # Имя каталога чистит `_slug`, и чистит честно: из «../../etc/passwd»
    # выходит «etcpasswd», из «..» — «project». Но защита эта регулярочная, а
    # CodeQL регулярку санитайзером не считает и зовёт путь «уязвимым» —
    # четыре находки высокой важности на PR #417, все ниже этой двери
    # (`_daily_dir`, `store_daily_report`). Кричащая зря проверка хуже
    # отсутствующей, а спорить с ней нечем: доказательство «подстрока не
    # пройдёт» живёт в голове читателя, а не в коде. Поэтому рядом стоит
    # замок, который проверяет РЕЗУЛЬТАТ: имя проекта — один сегмент прямо
    # под каталогом снимков, и никак иначе. Он же переживёт ослабление
