...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.40")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.40",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.40"
...
def _mo_market_price_for(district: str) -> tuple[float | None, str, str]:
    table = _mo_market_price_table()
    price = table["prices"].get(_mo_normalize_name(district))
    return price, table.get("period", ""), table.get("document", "")
...
def mo_reference() -> dict[str, Any]:
    ...
    return {
        ...
        "market_price": {
            "count": len(_mo_market_price_table()["prices"]),
...
def mo_calculate(req: MoCalculateRequest) -> dict[str, Any]:
    ...
    if market_price <= 0:
        ...
        official, market_price_period, market_price_document = _mo_market_price_for(upks["district"] or district)
        if official:
            market_price = official
            market_price_source = "справочник Комитета по ценам и тарифам МО"
            warnings.append(
                "Кср взят из справочника средней рыночной стоимости 1 м² жилья"
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.40",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.40"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.40 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
