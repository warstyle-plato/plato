...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.74")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.74",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.74"
...
def mo_calculate_via_core(query: str, limit: int = 30) -> dict[str, Any]:
    """Единая точка входа бота: удалённое ядро, если оно задано, иначе локальный расчёт."""
    if _MO_CALC_API_URL:
        return _mo_calculate_remote(query, limit)
...
def land_lookup_via_core(query: str, limit: int = 30) -> dict[str, Any]:
    """Поиск участка по адресу, номеру или координатам — там же, где доступен НСПД."""
    url = _core_api_url("/land/lookup")
    if url:
        return _core_post(url, {"query": query, "limit": int(limit)}, _MO_CALC_TIMEOUT_SECONDS)
...
_TELEGRAM_WEB_APP_BASE_URL = (
    os.environ.get("TELEGRAM_WEB_APP_BASE_URL") or _TELEGRAM_PUBLIC_BASE_URL
).rstrip("/")
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.74",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.74"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.74 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
