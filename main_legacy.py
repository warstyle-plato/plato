...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.50")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.50",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.50"
...
def _model_sheet_phase_comparison(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    if comparison:
        ...
        charts.append({
            ...
            "series": [{
                "name": "LLCR",
                "values": _xlsx_sheet_ref("Сравнение очередей", "I", first_data_row, last_data_row),
            }],
        ...
        charts.append({
            ...
            "categories": names,
            "series": [
                {"name": "Выручка", "values": _xlsx_sheet_ref("Сравнение очередей", "C", first_data_row, last_data_row)},
                {"name": "CAPEX", "values": _xlsx_sheet_ref("Сравнение очередей", "D", first_data_row, last_data_row), "color": "B4762A"},
            ],
...
def _plato_tep_value(tep: dict[str, dict[str, Any]], path: str) -> float | None:
    ...
    return _land_float((tep.get(product) or {}).get(field))
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.50",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
VRI_DEFAULTS: dict[str, Any] = {
    ...
    "vri_obligation_date_mode": "at_rns",  # at_rns | before_rns_1m | before_rns_3m | after_purchase | manual
...
def vri_obligation_date(x: dict[str, Any], permit: date) -> tuple[date, str, bool]:
    ...
    """
    mode = str(x.get("vri_obligation_date_mode") or "at_rns").strip().lower()
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.50"}
...
def calculate_phased(req: PhasedCalcRequest) -> dict[str, Any]:
    ...
    social_objects = copy.deepcopy(phasing.get("social_objects") or [])
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.50 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
  ...
  <div class="content">
    <div id="inputs" class="panel active">
      ...
      <div class="card import-card">
        ...
        <div class="cadastral-box mo-box">
          ...
          <div class="mo-price-line">
            <span id="moPriceState">Справочник Кср не загружен.</span>
            <input type="file" id="moPriceFile" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
            <input type="text" id="moPricePeriod" placeholder="период, напр. III–IV кв. 2025" style="max-width:210px">
            <button class="btn" onclick="uploadMoPrices()">Загрузить таблицу Кср</button>
          </div>
          <div style="font-size:11px;color:#888;margin-top:5px">Приложение к распоряжению Комитета по ценам и тарифам Московской области о средней рыночной стоимости 1 м² жилья по муниципальным образованиям. Если справочник загружен, Кср подставляется по округу автоматически.</div>
    ...
    <div id="vri" class="panel">
      <div class="card">
        ...
        <div class="note">Платежи до открытия ПФ несёт БРИДЖ или собственный капитал, после — ПФ, и только если ВРИ включена в банковский бюджет. Проценты по рассрочке считаются отдельно от процентов по кредитам.</div>
...
async function loadMoReference(){
 try{
  ...
  renderMoPriceState(data.market_price||{});
 }catch(e){}
}

function renderMoPriceState(state){
 ...
 node.innerHTML=count
  ?'<b>Справочник Кср: '+count+' муниципальных образований</b>'+(state.period?' · '+escapeHtml(state.period):'')+
   ...
   '. Кср подставляется по округу автоматически.'
  :'Справочник Кср не загружен — Кср берётся из поля выше или из УПКС округа.';
}

async function uploadMoPrices(){
 const input=document.getElementById('moPriceFile');
 const status=document.getElementById('moStatus');
 const file=input&&input.files&&input.files[0];
 if(!file){status.innerHTML='<span class="import-error">Выберите файл приложения к распоряжению.</span>';return}
 status.textContent='Разбираю таблицу средней рыночной стоимости…';
 try{
  const period=encodeURIComponent(document.getElementById('moPricePeriod').value||'');
  const response=await fetch('/mo/market-price/import?period='+period,{method:'POST',body:file});
  const data=await response.json();
  if(!response.ok)throw new Error(data.detail||'Не удалось разобрать таблицу');
  renderMoPriceState({count:(data.rows||[]).length,period:data.period});
  const notes=(data.warnings||[]).map(x=>'<br>• '+escapeHtml(x)).join('');
  status.innerHTML='<span class="import-ok">Справочник Кср загружен: '+(data.rows||[]).length+
   ' строк, сопоставлено округов — '+data.matched_districts+'.</span>'+notes;
 }catch(e){
  status.innerHTML='<span class="import-error">'+escapeHtml(String(e.message||e))+'</span>';
 }
}
