...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.82")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.82",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.82"
...
def _looks_like_address(text: str) -> bool:
    ...
    if not value or len(value) > 300:
        return False
    if _TEP_TEXT_MARKERS.search(value):
        return False
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.82",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.82"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.82 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
...
async function applyMo(options){
 ...
 glavapuImport=null;
 delete inputs._manual_tep_import;
...
async function applyGlavapu(){
 ...
 // Источник ТЭП ровно один. Метки прошлых источников раньше оставались, и
 // московский участок по ГлавАПУ приходил в чат подписанным «Ручной шаблон
 // DevelopAid», а отчёт — именем округа из прошлого расчёта по Подмосковью.
 delete inputs._manual_tep_import;
 delete inputs._mo_calc;
 moResult=null;
 // Площадь территории ГлавАПУ знает точно. Раньше она оставалась «справочной»,
 // а в inputs продолжала лежать площадь прошлого проекта — и карточка в чате
 // показывала её вместо площади текущего участка.
 {
 ...
 // Продукты, которых ГлавАПУ не отдал, обнуляем: раньше они оставались от
 // прошлого проекта. Так в московский расчёт попали 833 кладовые из чужого
 // ТЭП — миллиард выручки при нулевой площади, то есть нулевой себестоимости.
 const glavapuTep=glavapuImport.mappings.tep||{};
 ['storage','standalone_retail','offices','above_parking'].forEach(key=>{
  if(glavapuTep[key]||!tep[key])return;
  ['gns','total_area','useful','saleable','transfer','units'].forEach(field=>{
   if(field in tep[key])tep[key][field]=0;
  });
 });
 const glavapuInputs=glavapuImport.mappings.inputs||{};
 if(!('offices_enabled' in glavapuInputs))inputs.offices_enabled=false;
 inputs.retail_enabled=false;
 inputs.above_parking_enabled=false;

 Object.entries(glavapuTep).forEach(([key,vals])=>{
...
async function applyTelegramManualTep(manual,options){
 ...
 // Территория прошлого проекта хранится в браузере отдельно от ТЭП и раньше
 // переживала загрузку нового: приходил расчёт по 22 участкам Подмосковья, а
 // блок «Участок» продолжал показывать один московский номер из прошлого раза.
 delete inputs._cadastral_analysis;
 delete inputs._mo_calc;
 moResult=null;
 tep=structuredClone(TEP_DEFAULT);
 phasing=makeDefaultPhasing(1);
