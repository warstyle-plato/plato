...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.91")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.91",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.91"
        ...
        try:
            ...
            try:
                _telegram_send_message(
                    ...
                    f"<i>{html.escape(str(reason)[:300])}</i>\n\n"
                    "Карточка и PDF выше посчитаны и верны — не хватает только выгрузки. "
                    "Её можно скачать кнопкой «Выгрузить модель» в мини-приложении.",
                )
            except Exception:
                pass
    return {"ok": True}
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.91",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.91"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.91 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
...
async function sendTelegramResult(){
 ...
 try{
   ...
   if(telegramMode==='edit'){
     ...
     finishTelegramSession('Итоговая карточка и PDF-отчёт отправлены в чат.');
   }
 }catch(e){
   const status=document.getElementById('glavapuStatus');
