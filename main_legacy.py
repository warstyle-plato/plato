...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.49")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.49",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.49"
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.49",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.49"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.49 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
...
function renderVri(vri){
 ...
 document.getElementById('vriTotalsTable').innerHTML=
   (Number(t.relief||0)>0?row('Обязательство до льготы',money(t.gross))+row('Льгота',money(t.relief)):'')+
