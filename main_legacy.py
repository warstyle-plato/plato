...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.63")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.63",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.63"
...
# По одному адресу в ЕГРН стоят сотни помещений: квартиры, машино-места,
# кладовые. Для расчёта проекта нужен участок, поэтому помещения из поиска
# по адресу убираются — но не молча, а с указанием, сколько их скрыто.
_NSPD_PREMISE_WORDS = (
...
def _nspd_object_kind(feature: dict[str, Any], options: dict[str, Any]) -> str:
    ...
    if any(word in label for word in _NSPD_PREMISE_WORDS):
        return "premise"
    if any(word in label for word in ("здан", "сооруж", "объект недвиж", "окс", "строен")):
        return "building"
...
def _land_lookup_features_to_results(
    features: list[dict[str, Any]], limit: int, *, include_premises: bool = True
) -> tuple[list[dict[str, Any]], int]:
    """Нормализованные объекты ЕГРН и число скрытых помещений."""
    normalized = [_normalize_nspd_feature(item) for item in features]
    premises = [item for item in normalized if item["kind"] == "premise"]
    if not include_premises:
        normalized = [item for item in normalized if item["kind"] != "premise"]
...
def land_lookup(req: LandLookupRequest) -> dict[str, Any]:
    ...
    # квартира, прятать её нельзя. По адресу и точке помещения убираем —
    # там их сотни, а нужен участок.
    include_premises = bool(getattr(req, "include_premises", False))
    numbers: list[str] = []
    seen: set[str] = set()
    ...
    if numbers:
        ...
        if coordinates:
            ...
            results, hidden = _land_lookup_features_to_results(
                _nspd_point_features(lat, lng), limit, include_premises=include_premises
            )
            ...
            try:
                ...
                warnings.append(str(exc.detail))
            results, hidden = _land_lookup_features_to_results(
                features, limit, include_premises=include_premises
            )
            hidden_premises += hidden
            if not results:
                ...
                for candidate in candidates:
                    if candidate.get("cadastral_number"):
                        ...
                        found, hidden = _land_lookup_features_to_results(
    ...
    if hidden_premises:
        warnings.append(
            f"По этому адресу в ЕГРН {hidden_premises} помещений — квартиры, машино-места, "
            "кладовые. В выдаче они скрыты: для расчёта проекта нужен земельный участок. "
            "Чтобы увидеть помещение, введите его кадастровый номер."
        )
    warnings.append(
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.63",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.63"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.63 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
