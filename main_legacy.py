...
# Единственное место, где живёт номер версии. Раньше он был размножен по
# тринадцати литералам, и обновлялись они порознь: бот показывал одно, `/health`
# и страница — другое, а «модель не развернулась» и «версию забыли поднять в
# одном месте из тринадцати» выглядели одинаково. Обёртка `main.py` берёт
# значение отсюда же, поэтому источник один на оба файла.
VERSION = "0.13.6"
