...
app = FastAPI(title="DevelopAid Development Investment Model", version="0.12.52")
...
def analyze_cadastral_territory(req: CadastralAnalysisRequest) -> dict[str, Any]:
    ...
    external_request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "DevelopAid-Development-Model/0.12.52",
        },
    )
    try:
...
_LAND_LOOKUP_USER_AGENT = "DevelopAid-Development-Model/0.12.52"
...
def build_model_archive(
    ...
    *,
    ...
    if not phased:
        ...
        archive_files.append(("90_Детализация_консолидация.xlsx", _build_model_xlsx(consolidator_sheets)))
        for index, phase in enumerate(phases, start=1):
...
def fetch_current_cbr_key_rate() -> dict[str, Any]:
    ...
    try:
        ...
        req = urllib.request.Request(
            feed_url,
            headers={
                "User-Agent": "Mozilla/5.0 DevelopAid-Development-Model/0.12.52",
                "Accept-Language": "ru-RU,ru;q=0.9",
            },
        )
...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "version": "0.12.52"}
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.12.52 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
    ...
    <div id="inputs" class="panel active">
      <div class="card import-card">
        ...
        <details class="cadastral-box mo-box" id="moParamsBox">
          ...
          <p>Заполняются из справочников автоматически. Меняйте, только если знаете фактические значения по проекту — введённое всегда важнее справочного.</p>
          <div class="mo-params">
            ...
            <div class="field"><label>Средняя цена м², Кср <span class="unit">₽/м² · из справочника</span></label><input type="number" id="moPrice" value="" step="1000" placeholder="по округу автоматически"></div>
...
function syncMoParams(data){
 ...
 if(district&&t.district)district.value=t.district;
 const price=document.getElementById('moPrice');
 if(price&&v.market_price_rub_per_sqm)price.value=Math.round(v.market_price_rub_per_sqm);
