...
function sorted(v){return [...v].sort((a,b)=>(b.score!=null)-(a.score!=null)||(b.score??-1)-(a.score??-1)||(b.coverage_pct??0)-(a.coverage_pct??0)||(b.llcr??-1)-(a.llcr??-1)||a.lot-b.lot)}
function render(){let v=sorted(rows);$('cards').innerHTML=v.map(card).join('')||'<div class="note">Данные ещё собираются.</div>';v.forEach(x=>{if(detailCache[x.lot])drawDetail(x.lot,detailCache[x.lot]);else quickDetail(x.lot)});$('state').textContent='12 площадок · полный рейтинг: '+v.filter(x=>x.score!=null).length+' · расчёт/сбор данных: '+v.filter(x=>x.working).length}
...
async function quickDetail(lot){try{let d=await fetch('/krt-12-preview/lot/'+lot).then(r=>r.json());if(d&&d.available){detailCache[lot]=d;drawMap(lot,d.map||{});let el=$('detail-'+lot);if(el&&el.closest('details').open)drawDetail(lot,d)}}catch(e){}}
async function loadDetail(lot){let el=$('detail-'+lot);if(detailCache[lot]&&detailCache[lot].available){drawDetail(lot,detailCache[lot]);return}try{let d=await fetch('/krt-12-preview/lot/'+lot).then(r=>r.json());if(d&&d.available)detailCache[lot]=d;drawDetail(lot,d);if(d&&d.available)drawMap(lot,d.map||{})}catch(e){el.innerHTML='<div class="note bad">'+esc(e.message)+'</div>'}}
