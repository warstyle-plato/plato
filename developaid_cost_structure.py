...
def build_cost_structure_matrix(region: str = "Москва", housing_class: str = "business") -> dict[str, Any]:
    ...
    for raw in structure.get("sources", []):
        if raw.get("region") != region:
            continue
        source = deepcopy(raw)
        # Адрес или имя собственного проекта — коммерческая информация:
        # страница /statistics и её API открыты, и один раз адрес уже утёк на
        # пользовательскую поверхность (решение владельца, 26.08.2026 — убрать).
        # Наружу внутренний источник выходит обезличенным; сырые данные в
        # reference_data не трогаются.
        if str(source.get("source_kind") or "") == "internal_project":
            source["source"] = "Внутренний проект DevelopAid"
            # Идентификаторы тоже несут адрес латиницей — наружу выходит
            # детерминированный хэш, чтобы связки между строками сохранились.
            for id_key in ("source_id", "source_group"):
                raw_id = str(source.get(id_key) or "")
                if raw_id:
                    source[id_key] = "developaid-internal-" + hashlib.sha1(
                        raw_id.encode("utf-8")).hexdigest()[:10]
