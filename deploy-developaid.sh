...
start_container() {
  ...
  # Корневые сертификаты, которых нет в системном хранилище, монтируются
  # рядом с данными. Российские госсайты (ГИС Торги) выпускают сертификаты у
  # национального удостоверяющего центра, и его корня в образе нет. Через
  # монтирование корень кладётся без пересборки: до этого файлы на машине
  # лежали, а в контейнер не попадали вовсе — «положил и не работает».
  mkdir -p "$ROOT/certs"
