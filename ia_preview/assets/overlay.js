...
(function () {
  ...
  function annotateTep() {
    ...
    if (!calcBar) {
      ...
      // Московскому участку блока нет вовсе: кнопка с подписью «не нужна»
      // всё равно нажимается — и молча заменяет данные калькулятора
      // формульными (владелец нажал и спросил «зачем?»). Править ТЭП такого
      // участка можно прямо в таблице — эта возможность остаётся.
      manualWrap.style.display = moscowLoaded ? 'none' : '';
      // Заголовок говорит, кому блок нужен; базовая посадка — из поля
      // страницы, не копией: цифра, разъехавшаяся с полем, хуже отсутствующей.
      ...
      manualWrap.querySelector('summary').textContent = mo
