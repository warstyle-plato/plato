...
def test_bundled_registry_ships_with_the_code() -> None:
    ...
    # Граница справочника сдвинулась вместе с источником, и утверждение о ней
    # пришлось переписать. Прежде отчёт приходил PDF-ом за пару месяцев, и
    # «Хамовники XII» в нём не было — дом почти достроен и ДДУ у него единичные.
    # Месячная книга «Пульса» несёт ряд за двадцать четыре месяца по каждому
    # проекту, поэтому такой дом в справочнике ЕСТЬ, а месяцы без сделок стоят
    # нулями. Это не дефект: ноль здесь — измеренный ответ «в этом месяце не
    # продавали», и он честнее отсутствия строки.
    khamovniki_xii = registry.find("Хамовники 12")
    assert khamovniki_xii is not None, "проект с редкими ДДУ теперь в окне отчёта"
    assert khamovniki_xii.district == "Хамовники"
    assert any(value == 0 for value in khamovniki_xii.sales.values()), (
        "месяц без сделок обязан стоять нулём, а не пропуском"
    )
    # А настоящая граница осталась прежней: кого в отчёте нет вовсе, того нет и
    # здесь — таких приносит поиск, а не справочник.
    assert registry.find("Небывалый Квартал На Луне") is None
