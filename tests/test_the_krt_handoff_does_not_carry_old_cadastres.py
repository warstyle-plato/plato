...
**У площадки КРТ поле участка оставалось от ПРОШЛОГО проекта.** Чужой номер
хуже пустого поля: он выглядит посчитанным. Тогда это чинилось очисткой, и
рядом стояло объяснение «у площадки КРТ номеров нет вовсе» — верное в тот
день и неверное с тех пор, как контур карточки собирается по перечню проекта
решения. Решение называет номера поимённо: на Варшавском ш., вл. 37 их 60, из
них 20 участков и 39 зданий. Теперь поле заполняется участками, а здания
отсекает тот же ЕГРН, что отсекает их у лота.
...
def test_the_site_handoff_fills_the_field_from_the_decision() -> None:
    """Поле участка заполняется перечнем решения, а пустое — объяснено."""
    assert "Кадастровых номеров у площадки не прочитано" in BRIDGE, \
        "пустое поле не объяснено причиной"
    ...
    assert "pending.krt_cadastres" in krt, "перечень решения до поля не доезжает"
    # Строго после подмены проекта: она забывает территорию прошлого участка
    # целиком, и заполненное до неё исчезло бы.
    assert krt.index("applyProjectSnapshot(model)") < krt.index("field.value=cads.join"), \
        "поле заполняется до подмены проекта — она его сотрёт"
