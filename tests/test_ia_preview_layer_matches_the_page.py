...
Отдельно закреплена перегруппировка вкладок страницы в разделы: вкладка, не
попавшая ни в один раздел, исчезает со страницы целиком — вместе со своими
полями и своим куском модели. Числа вкладок и разделов здесь нет намеренно —
их пересказ устаревает молча, а состав сверяется с самой страницей.
...
# Собранное приложение, а не голая обёртка: слой живёт на той странице, которую
# отдаёт `/`, а её досоставляют расширения — вкладку «Рынок» ставит market_search.
# Пока тест читал `main`, вкладка попадала в PAGE только если main_registry успел
# импортироваться раньше в том же прогоне: потеря вкладки ловилась через раз и
# зависела от порядка файлов.
