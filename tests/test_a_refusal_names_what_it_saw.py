...
def test_a_registry_answer_is_named_as_one() -> None:
    ...
    Подпись «Кадастровый номер:» стоит на КАЖДОМ листе формы, поэтому гасить
    её надо во всех семи: погашенная в одной, она оставляет номер в шапке
    следующего листа — и разбор верно читает его оттуда. Прежде тут стояло
    `count=1`, и отказ держался на другом: клетка второго листа склеена с
    остатком блока подписи, а гейт требовал совпадения по ВСЕЙ клетке. Гейт
    ослаблен (номер берётся из начала клетки, 16.09.2026), и пример стал
    проверять не своё утверждение.
    """
    text = re.sub(r"(Кадастровый\s+номер\s*:\s*\n)77:05:0012007:2054",
                  r"\1данные отсутствуют", egrn_fixtures.LIVE)
    with pytest.raises(ValueError) as caught:
        pf.read_text(text)
