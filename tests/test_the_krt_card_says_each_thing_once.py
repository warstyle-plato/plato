...
def test_the_named_number_stands_next_to_its_dash():
    """Прочерк и названное число отвечают на один вопрос — значит стоят рядом.

    «Почему тут нет жилья якобы в системе?» (владелец, 21.09.2026, Рубцовская
    наб., влд. 3): в паспорте «Жильё — решение не называет» и «Нежилое по
    решению — решение не называет», а двумя строками ниже «Квартиры по решению
    4 290 м²» и «Нежилая наземная 16 200 м²». Жильё при этом посчитано: модель
    восстанавливает жилой объём из площади квартир той же долей, которой
    считает продаваемую, — и сказано об этом было только в предпосылках модели,
    десятью блоками ниже. Второй раз одно и то же чтение: 08.09.2026 — «как это
    нет данных и сразу есть?», и тогда починили гейт расчёта, а подпись нет.

    Числа подсказка не считает: она называет, куда смотреть.
    """
    html = _render("krtPassport", DECISION).replace(" ", " ")
    order = [html.index("<div>Жильё</div>"),
             html.index("<div>Квартиры по решению</div>"),
             html.index("<div>Нежилое по решению</div>"),
             html.index("<div>Нежилая наземная</div>")]
    assert order == sorted(order), f"названное число уехало от своего прочерка: {order}"

    housing = html[order[0]:order[1]]
    assert "решение не называет" in housing, housing
    assert "модель считает по площади квартир" in housing, housing
    # Подсказка — не число: площадь квартир печатает своя строка.
    assert "4 290" not in housing and "15 681" not in housing, housing

    # Названо есть чем — значит подсказки нет: обещать строку, которой не
    # будет, хуже молчания.
    lonely = _render("krtPassport", {k: v for k, v in DECISION.items()
                                     if k not in ("flats_sqm", "nonresidential_ground_sqm")})
    assert "модель считает по площади квартир" not in lonely, lonely
    assert "решение не называет" in lonely, lonely
