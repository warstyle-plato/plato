...
стоит она у нормы площади двора: это единственное место, где человек этот
метраж и правит. Первой строкой справка говорит, ЧЬЁ число в поле: наши
5/15/20 м²/чел. — стоимость двора по классу (решение владельца, 12.09.2026), а
не норма города, и одно под другим читалось бы как ссылка на акт там, где
стоит экспертная ставка.
...
FIELD = "f_landscaping_area_per_person_sqm"
...
def test_the_note_reaches_the_screen(seen):
    assert seen["msk"]["field"], "поля норматива благоустройства на странице нет"
...
def test_the_note_is_gone_outside_moscow(seen):
    ...
    assert seen["mo"]["field"], "в Подмосковье поля норматива нет вовсе — меряем не то"
