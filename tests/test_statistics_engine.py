...
def test_cost_structure_matrix_uses_developaid_rows_and_keeps_units():
    ...
    # Внутренний источник наружу выходит обезличенным (см. маскировку в
    # build_cost_structure_matrix): идентификатор — детерминированный хэш.
    internal = by_id["developaid-internal-cc04b061d6"]
    assert internal["cells"]["main_above"]["adjusted_value_rub_m2"] == 168816.78
    assert internal["cells"]["main_above"]["unit"] == "above_ground"
    assert internal["cells"]["main_under"]["unit"] == "underground"
