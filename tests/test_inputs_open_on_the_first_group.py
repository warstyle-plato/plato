...
def test_a_repeated_unit_is_printed_once():
    ...
    У наземной и подземной части базы разные — рубли на метр своей части, — и
    единица у каждой своя: складывать их в одну нельзя, они про разное. Но
    одинаковая единица дважды подряд по-прежнему запрещена."""
    peek = {item["name"]: item["peek"] for item in render(
        {"main_above_th_per_sqm": 190, "main_under_th_per_sqm": 120})}
    line = peek["Строительство"]
    assert line == "190 тыс. ₽/м² наземной части · 120 тыс. ₽/м² подземной части"
    for unit in ("тыс. ₽/м² наземной части", "тыс. ₽/м² подземной части"):
        assert line.count(unit) == 1, unit
