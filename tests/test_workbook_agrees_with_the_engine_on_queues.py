...
def engine_queue_totals(inputs, tep, count) -> dict[str, float]:
    ...
    for phase in bundle["phases"]:
        ...
        # Книга получает ПРОДАВАЕМОЕ количество: ячейка идёт в объём продаж, а
        # гостевые места строятся, но не продаются. Сравнивать её с построенным
        # числом значило бы требовать от книги продавать несуществующее.
...
def test_both_keep_the_project_total(project):
    ...
    # 2 723 места построено, гостевые из них не продаются — в книгу уезжает
    # продаваемое количество, и оно же складывается по очередям.
