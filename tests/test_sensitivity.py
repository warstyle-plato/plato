...
def test_the_verdict_flags_a_broken_llcr_target():
    # Цена ходит вслед за умолчаниями, и в обе стороны. Сперва её поднимали:
    # после сверки удельных ставок с банковским бюджетом стройка подорожала, и
    # 620 не давали дотянуть до цели. Теперь опускаем: с 0.23.17
    # благоустройство считается по площади двора, а не по строительному
    # объёму, проект подешевел, и на 680 база встала на 1,327 — отклонение в
    # 10% цель уже не сбивает, то есть тест перестал проверять то, ради чего
    # написан. Утверждение у него одно: база держит цель, а отклонение фактора
    # её ломает, — и на 640 держатся оба конца (1,265).
    inputs, tep = project(apartment_price_th=640)
    report = core.run_sensitivity(inputs, tep, [], {}, metric="llcr")
