...
def test_the_city_chain_starts_from_spp(guide: str) -> None:
    ...
    # Прежде проверка держала слово «неверное»: руководство называло ошибочное
    # утверждение и помечало его. Нынешняя редакция его не повторяет вовсе, а
    # основание отрицает прямо — это то же утверждение и более сильная форма.
    assert "не от общей площади" in body, "не сказано, что база — не общая площадь"
