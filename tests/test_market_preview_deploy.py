...
def test_preview_health_json_is_passed_as_argument() -> None:
    script = Path("deploy-market-preview.sh").read_text(encoding="utf-8")
    health = script.split("health_check()", 1)[1].split("route_check()", 1)[0]
    assert "json.loads(sys.argv[2])" in health
    assert "json.load(sys.stdin)" not in health
