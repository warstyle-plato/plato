...
Проверяется то, что видно: строка списка несёт кнопку «Сохранить» со своим id, а
шапка — две названные кнопки. Имя кнопки строки — решение владельца
(06.09.2026: «надо было сохранить назвать, а не поверх»): «Поверх» отвечает на
«как записать», а человек ищет «куда сохранить», и строка уже называет проект. Строковый поиск по `PAGE` тут не годится — он
...
def test_every_saved_row_offers_writing_over_it() -> None:
    html = _rendered()
    assert html.count(">Сохранить<") == len(ROWS), "кнопка стоит у каждой строки"
    assert 'data-id="abc123"' in html and 'data-id="row42"' in html, \
        "кнопка знает, поверх чего пишет"
    assert html.index(">Открыть<") < html.index(">Сохранить<"), \
