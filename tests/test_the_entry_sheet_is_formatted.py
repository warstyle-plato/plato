...
законный, и до этой правки ответ был «нет»: переезд ввода сделан, а вид листа
нет. Ширин колонок у него не было ВОВСЕ — единственный такой лист в книге, —
и колонка A, где лежат все названия строк, открывалась восемью символами:
подписи обрезаны. Собственная шапка листа и заголовки блока очередей шли без
стиля, то есть шрифтом по умолчанию посреди оформленной книги.

Проверяется не чтением исходника, а СОБРАННОЙ книгой: ширины и стили живут в
XML листа, и строка в коде о них ничего не доказывает.

Правило, которое здесь держится: **ширины и цвета берутся у шаблона, а не
выдумываются.** Своя ширина была бы вторым ответом на «сколько места нужно
подписи», а свой цвет сказал бы то, чего шаблон не говорит: цвет в этой книге
...
def test_the_entry_sheet_carries_the_template_widths(book) -> None:
    """Ширины колонок — шаблонные, вербатим.

    Ячейка переезжает СТРОКОЙ, а не колонкой: буквы колонок те же самые, значит
    и ширины подходят те же. Пока их не было, лист ввода открывался шириной по
    умолчанию, и колонка A шаблона (43 символа) показывала восемь.
    """
    entry = _sheet(book, ves.ENTRY_SHEET)
    template, _ = _template_sheet()
    want = re.search(r"<x:cols>.*?</x:cols>", template, re.S)
    assert want, "у шаблонного листа вводных нет ширин — брать их неоткуда"
    assert want.group(0) in entry, "лист ввода собран без ширин шаблона"
    first = re.search(r'<x:col min="1"[^>]*width="([\d.]+)"', entry)
    assert first and float(first.group(1)) > 20, (
        "колонка подписей уже двадцати символов — названия строк обрезаны")
...
def test_the_book_still_opens(book) -> None:
    ...
    assert loaded[ves.ENTRY_SHEET].column_dimensions["A"].width > 20
