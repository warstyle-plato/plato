...
def test_the_table_shows_renovation_and_the_builder():
    ...
    """
    page = ui.auctions_page(None)
    # Метка реновации собирается в krtMarks и с 0.21.77 называет ДОЛЮ: «реновация
    # 10,0%» или «реновация — всё жильё» — 15 100 м² из 150 940 и вся площадка
    # это разные новости. Искать её литералом «>реновация</span>» больше нельзя,
    # да и не надо: проверяется, что метку строит строка таблицы.
    assert "krtMarks(" in page, "метки строки таблицы никто не собирает"
    assert "реновация" in page, "метки реновации в строке таблицы нет"
