...
def test_the_normative_registry_is_reachable_from_the_calculator() -> None:
    """«И где же пользователь может увидеть ссылку на этот перечень?»
    (владелец, 07.09.2026).

    Страница `/normatives` существует и отвечает 200, и на ней ровно то, о чём
    спрашивают: какая редакция учтена, на какое правило или число она влияет и
    в каком модуле DevelopAid применяется. Дойти до неё было можно только из
    руководства или по ссылке в сообщении бота об изменениях — с калькулятора,
    где эти нормы и применяются, ни одной ссылки не было. Маршрут, до которого
    не дойти, — это маршрут, которого нет.

    Держится утверждение, а не место: подвал общий на все поверхности, значит
    ссылка обязана быть в разобранном составе — тогда она приезжает и на
    торги, и в кабинет, и в документы, а не только на главную.
    """
    import guide  # состав подвала разбирается из PAGE, а не переписывается сюда

    links = dict((href, label) for href, label in guide.legal_links(core))
    assert "/normatives" in links, links
    assert "норматив" in links["/normatives"].lower(), links["/normatives"]
