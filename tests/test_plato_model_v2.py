...
def test_changing_a_spread_moves_the_report(book):
    ...
    # покрытом проекте два пункта спреда двигают проценты на считаные проценты —
    # абсолютный порог мерил бы не проходимость вводной, а покрытие.
    assert after > was * 1.01, f"плюс 2 п.п. к спреду ПФ не сдвинули проценты: {was} → {after}"
