...
def test_the_pdf_column_names_the_project_gns():
    ...
    # ставка на подземный метр (она — 190, во вводных). «ГНС» в ЭТОЙ колонке
    # неверно: её база включает подземную часть, а та в наземную не входит.
    assert "тыс ₽/м² строительного объёма" in source, \
        "без названия базы удельные СМР читаются как ставка на свой метр"
    # Запрещается МЕСТО, а не слово. Прежде «тыс ₽/м² ГНС» было запрещено во
    # всём файле, и запрет упёрся в верную подпись: у благоустройства
    # производная считается на НАЗЕМНОЙ ГНС, и назвать её так — правда.
    # Проверка вида «такой строки в файле быть не должно» чинится обходом, а
    # обход выглядит правкой, поэтому она держит теперь именно свои колонки.
    for line in source.splitlines():
        if "тыс ₽/м² строительного объёма" in line or '"Статья", "млн ₽"' in line:
            assert "тыс ₽/м² ГНС" not in line, (
                "подземная часть в ГНС не входит: " + line.strip()[:120])
