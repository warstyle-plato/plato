...
def later_sales_repay_the_rve_gap():
    """При 380 тыс. ₽/м² в РВЭ остаётся долг, но к концу он погашен.

    Цена подобрана под сценарий и однажды уже перестала его держать: после
    0.23.17 благоустройство стало считаться по площади двора, проект на 400
    тыс. ₽/м² подешевел на два миллиарда и стал закрывать долг прямо в РВЭ —
    проверка упала на верной работе. Отсюда предохранитель ниже: разрыв в РВЭ
    обязан быть, иначе проверять нечего.
    """
    inputs = {**core.DEFAULT_INPUTS, "apartment_price_th": 380}
