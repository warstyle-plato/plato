...
def test_the_card_prints_what_the_machine_found():
    ...
    # День документа — такая же часть найденного, как номер и цена, и без него
    # карточка печатала «Согласно распоряжению № ДГП-Р-54/26 от » (замер
    # 15.09.2026). Проверка молчала: она утверждала номер и цену, а дату нет.
    # Отметка 02.09 21:00 UTC — это 03.09 00:00 по Москве: день у городского
    # документа московский, и на такой отметке видно, что зона пришпилена.
    assert "3 сентября 2026" in said, said[:200]
