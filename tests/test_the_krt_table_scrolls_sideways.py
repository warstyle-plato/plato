...
описание: его и не было, была давка. Тринадцатую колонку — «Цена окружения» —
добавил #454, и число колонок здесь держится литералом намеренно: сменилось
число — пересчитывают запас ширины, а не молча правят проверку.

Запас поэтому меряется НА КОЛОНКУ, а не числом колонок. Прежний сторож держал
литерал «их двенадцать» и требовал пересчитать запас; 20.09.2026 колонка «Цена
окружения» стала тринадцатой, литерал упал — и упал он на РОВНО ТОМ, ради чего
писался, ничего не сказав про сам запас: 1360 px на тринадцать колонок это
104,6 px против прежних 113,3. Утверждение здесь — «колонке досталось не
меньше того, под что таблица рассчитана», и оно не стареет от добавленной
рядом колонки: добавили — вырастите `min-width`.
...
# Запас ширины таблицы КРТ (`table.wide{min-width}`) считан по этой мере на
# колонку. Число живёт здесь, а не в утверждении: сравнивается оно с тем, что
# намерил браузер, и падает ровно тогда, когда колонку добавили, а запас нет.
RESERVE_PER_COLUMN = 113
...
def test_every_column_is_reachable_on_a_desktop(tmp_path, width):
    ...
    assert got["content"] >= got["columns"] * RESERVE_PER_COLUMN, (
        f'{got["columns"]} колонок при запасе {got["content"]} px — это '
        f'{got["content"] / got["columns"]:.0f} px на колонку при '
        f'{RESERVE_PER_COLUMN}: добавили колонку — вырастите min-width таблицы')
    ...
    assert got["content"] > got["wrap"], (
        ...
        f"возникает, потому что {got['columns']} колонок сжаты по месту")
