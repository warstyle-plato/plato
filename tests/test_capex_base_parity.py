...
def test_the_capex_total_matches_the_engine():
    ...
    for index, base in enumerate((0, 34)):
        ...
        # Порог относительный. Расхождение живёт в разнесении S-кривой между
        # очередями: статьи, которые она размазывает (наружные сети, а следом
        # управление и резерв от них), у книги и движка режутся по очередям чуть
        # по-разному — на первой −0,05 млн, на второй +0,24, в сумме почти ноль.
        # Величина этой разницы пропорциональна самим статьям, поэтому абсолютный
        # порог мерил бы размер проекта, а не точность сходимости: он прошёл на
        # ставке сетей 7,5 и упал на 10,25 при неизменной методике.
        assert value == pytest.approx(engine, rel=1e-4), f"очередь {index + 1}"
