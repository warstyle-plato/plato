...
def test_the_page_shows_the_ladder_next_to_the_structural_block() -> None:
    # Термины владельца (03.09.2026): 1 079 по статьям — фактическая нехватка,
    # структурный — то, что остаётся после перераспределения. Слова
    # «структурный» на экране нет вовсе: оно уже читалось двумя способами.
    for label in ("От нехватки по статьям к дефициту на достройку",
                  "Нехватка по статьям РСС",
                  "Структурная часть: закрывается перераспределением внутри глав",
                  "= Нехватка по статьям после разрешённых перебросок",
                  "Структурная часть: заперто в лимитах статей, где работы не закончены",
                  "= Нехватка в РСС в целом",
                  "Сверх РСС по модели",
                  "= Не хватает на достройку"):
