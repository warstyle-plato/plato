...
def test_the_screen_list_is_what_gets_marked() -> None:
    ...
    # функции.
    # Сборщик ищется по тому, ЧТО ОН ДЕЛАЕТ, а не по имени: проверка стояла на
    # `def _krt_screen_list(` и упала, когда счёт переехал в соседнюю функцию,
    # отвечающую заодно на «почему список неполон», — то есть на правке, которая
    # ничего из утверждаемого не трогала (15.09.2026). Это уже второй раз здесь:
    # прежде она держала форму записи вызова.
    at = source.index("catalogue + decisions")
    head = source.rindex("\n    def ", 0, at)
    body = source[head:]
    body = body[: body.index("\n    def ", 1)]
