...
from developaid_statistics_page import install
...
def test_business_recommendation_keeps_exact_developaid_bases():
    ...
    # 210 000, а не 592 526: второе число пришло из промежуточной книги и
    # стояло на другом знаменателе. Правка жила тремя копиями — в загрузчике
    # справочника и двумя заплатками поверх готовой матрицы, — а этот тест
    # держал доисправленное значение и потому её не видел. Ставка объявлена
    # один раз, в самом справочнике.
    underground = by_key["main_under"]
    assert underground["recommended_rub_m2"] == 210000.0
...
def test_article_consensus_uses_multiple_independent_sources_not_simple_listing():
    ...
    # Внутренний источник выходит наружу обезличенным: адрес проекта —
    # коммерческая информация (решение владельца, 26.08.2026).
    assert {x["source_id"] for x in utilities["included_sources"]} == {
        "developaid-internal-cc04b061d6",
...
def test_recommendation_api_keeps_the_audit_chain_for_both_project_shapes():
    """Ответ API несёт цепочку «как опубликовано → статья → класс → база».

    Проверка была на первой странице свода, которой больше нет: три её
    редакции жили одна поверх другой и снимали друг друга с маршрута. Сама
    цепочка от страницы не зависела — она в ответе, там и проверяется.
    """
    import main_registry

    app = FastAPI()
    install(app, main_registry.core)
    client = TestClient(app)

    project_params = [
        {"region": "Москва", "class": "business",
         "gba_sqm": 60322, "sellable_sqm": 31526, "underground_gns_sqm": 12915},
        {"region": "Москва", "class": "business",
         "gba_sqm": 22032.9, "sellable_sqm": 13710,
         "above_ground_gns_sqm": 18403.9, "underground_gns_sqm": 3629},
    ]

    for params in project_params:
        payload = client.get("/api/statistics/cost-recommendation", params=params).json()
        ...
        chain = [row for row in payload["recommendations"] if row.get("included_sources")]
        assert chain, "ни одной статьи с источниками — цепочку не по чему проследить"

    first = client.get("/api/statistics/cost-recommendation", params=project_params[0]).json()
    assert first["target_areas"]["above_ground_gns_sqm"] == 47407
    second = client.get("/api/statistics/cost-recommendation", params=project_params[1]).json()
    assert second["target_areas"]["above_ground_gns_sqm"] == pytest.approx(18403.9, abs=0.01)
    assert second["target_areas"]["underground_gns_sqm"] == pytest.approx(3629, abs=0.01)
