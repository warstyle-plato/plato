...
def test_every_measure_is_charted_and_money_leads() -> None:
    """Меры раздела рисуются все и встают рядом — на один слайд.

    Тут побывали оба крайних решения. Лист на каждую меру («не проще для
    каждого графика свой слайд сделать?», владелец, 29.08.2026) дал три почти
    одинаковых столбиковых листа подряд — «пустые два слайда» (31.08.2026).
    Ответ «график на раздел один, остальные названы подписью» оказался хуже:
    названная словом мера в документе отсутствует — «в презентации шаблона
    есть 3 графика на слайде 2. У тебя 1» (владелец, 03.09.2026). Верный ответ
    — ширина: графики стоят в ряд на одном слайде, как в шаблоне.
    """
