...
def test_auctions_exposes_krt_as_a_separate_tab_and_endpoint(monkeypatch) -> None:
    ...
    # Вывод по рынку подписан тем, кто его считает: `analysis.site` — это
    # арифметика модуля рынка по соседям, а не ответ Платона. Пока блок звался
    # «Короткий вывод Платона», на карточке стояло два его мнения об одной
    # площадке, и одно из них было наше (владелец, 06.09.2026).
    # Запрещается ЗАГОЛОВОК, а не слово: объяснение, почему блок переименован,
    # стоит в коде рядом и содержит прежнее имя — проверка на слово завалилась
    # бы на собственном объяснении.
    assert "<h3>Что здесь строить и почём</h3>" in page.text
    assert "<h3>Короткий вывод Платона</h3>" not in page.text
