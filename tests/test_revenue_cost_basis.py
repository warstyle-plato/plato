...
С 14.09.2026 обычным путём такую строку не собрать: штуки и метры кладовых
стали парой и достраивают друг друга (`storage_tep_row`). Пример поэтому
взят там, где пара НЕ чинит, — строка, объявленная очередью: её движок не
трогает, потому что её площадь есть доля уже посчитанной проектной. Сторож
остаётся: правило шире кладовых, а недостижимый сегодня пример завтра
приедет чужим источником. Соседняя проверка держит вторую половину — что
обычным путём пара достраивается и кричать не о чем.
...
def project(storage_units: float):
    ...
    # Строка очереди: пара «штуки ↔ метры» её не достраивает, и продукт с
    # количеством без площади остаётся возможным — ровно тот случай, ради
    # которого сторож и написан.
    tep["storage"][main.TEP_ROW_DECLARED] = True
