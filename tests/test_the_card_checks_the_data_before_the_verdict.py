...
def test_the_card_says_the_check_before_the_score() -> None:
    ...
    # И место: проверка, которой ЕСТЬ что сказать, стоит своей плашкой сразу за
    # паспортом — до раскрытий; молчащая живёт ВНУТРИ «чего не хватает», а не
    # постоянной припиской наверху (на живом каталоге таких строк 501 из 580).
    # Сравнивать её с баллом больше нельзя: с 0.22.25 порядок карточки задал
    # владелец, и весь нормативный блок стоит ВЫШЕ вердикта — «Балл площадки»
    # ниже обоих ответов по построению, и такая проверка подтверждала бы себя.
    gaps = "Что про площадку известно и чего не хватает"
    assert agrees.index("Проверка данных") < agrees.index(gaps), agrees
    assert differs.index("Проверка данных") < differs.index(gaps), differs
