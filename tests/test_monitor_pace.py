...
def test_pace_finish_uses_recent_acts_rate():
    ...
    # 75% remaining / 5% per month = 15 months from cut.
...
def test_management_aggregate_keeps_child_forecast_delta():
    nodes = [{
        "level": "corpus",
        "plan_finish": "2027-06-30",
        "children": [
            {"level": "task", "plan_finish": "2027-05-31", "forecast_finish": "2027-07-15", "pace_forecast_known": True},
            {"level": "task", "plan_finish": "2027-06-30", "forecast_finish": "2027-06-30", "pace_forecast_known": False},
        ],
    }]
    pace._aggregate_management(nodes)
    assert nodes[0]["forecast_finish"] == "2027-07-15"
    assert nodes[0]["delta_days"] == 15
