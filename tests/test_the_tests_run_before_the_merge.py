...
# Худшая измеренная ДОЛЯ, минуты. Потолок стоит на доле, значит и мерить надо
# долю: первый зелёный прогон долями (34413322575 от 09.09.2026) дал
# 21:00 / 22:32 / 32:09 / 30:01 при 1:38:43 целиком. Берётся worst, округлённый
# вверх, — а не среднее: снимается по таймауту та доля, что не уложилась.
#
# Число это ПЛАВАЕТ вместе с составом `tests/`: раскладка считается по файлам,
# и накануне те же четыре доли шли 14,3 / 24,5 / 28,0 / 30,8. Поэтому запас
# двукратный, и поэтому же его мерят заново, а не помнят.
LAST_MEASURED_SHARD_MINUTES = 33
