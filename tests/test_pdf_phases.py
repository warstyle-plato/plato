...
def test_unit_metrics_are_shown(phased_text):
    """Каждый удельный показатель — в двух базах: на строительный объём и на
    продаваемую.
    ...
    # База удельных — весь строительный объём, наземная часть плюс подземная.
    # Подписывать её «ГНС» нельзя: подземная в наземную площадь не входит,
    # и показатель на метр выходит выше того же показателя по методике города.
    for column in ("Выручка на м² прод.", "Выручка на м² строит. объёма",
                   "Расходы на м² прод.", "Расходы на м² строит. объёма",
                   "Прибыль на м² прод.", "Прибыль на м² строит. объёма"):
