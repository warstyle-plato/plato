...
def load_external_benchmarks(path: Path | None = None) -> list[ExternalBenchmark]:
    # Справочник едет с образом, а не с диска: `data/` на ядре — примонтированный
    # том, и файл из образа под ним не виден вовсе. Записываемый путь остаётся
    # запасным — туда кладут пополнение, если оно появится.
    target = path or _reference_first("external_benchmarks.json")
    if not target.exists():
        return []
    return [ExternalBenchmark(**row) for row in json.loads(target.read_text(encoding="utf-8"))]


def _reference_first(name: str) -> Path:
    """Справочник образа, а под ним — записываемый том.

    Выверенные ряды лежат в `reference_data/` и едут с образом. `data/` на
    ядре — примонтированный том: файл из образа под ним не виден, поэтому
    справочник, прочитанный только оттуда, в проде оказался бы пустым, а
    пустой ряд от отсутствующего не отличить.
    """
    target = REFERENCE_DATA_DIR / name
    return target if target.exists() else DATA_DIR / name


def load_normalized_benchmarks(path: Path | None = None) -> list[NormalizedBenchmark]:
    target = path if path is not None else _reference_first("normalized_benchmarks.json")
    if not target.exists():
        return []
...
def _point_dict(point: NormalizedBenchmark) -> dict[str, Any]:
    ...
    # Адрес собственного проекта — коммерческая информация; наружу внутренний
    # источник выходит обезличенным (решение владельца, 26.08.2026).
    if str(payload.get("source_kind") or "") == "internal_project":
        payload["source"] = "Внутренний проект DevelopAid"
        raw_id = str(payload.get("external_id") or "")
        if raw_id:
            payload["external_id"] = ("developaid-internal-"
