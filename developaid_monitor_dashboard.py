...
def _summary(view: dict[str, Any], funding: dict[str, Any],
    ...
    # утверждённому ДДС» — источник назывался чужим именем, и с ним терялась
    # граница: «до конца стройки» звучит как ВСЁ, что стройке нужно, а это
    # инвестиционные расходы глав 2–3 и ничего сверх них (владелец,
    # 30.08.2026: «не по ДДС а по РСС, а так-то мы знаем, что денег нужно на
    # стройку гораздо больше»).
    out.append(
        ...
        f"{fuel / need * 100:.0f}% этой потребности. Это остаток по РСС, а не "
        f"вся потребность стройки: что вне глав 2–3 и вне книги, сюда не "
        f"входит." if need > 0 else
