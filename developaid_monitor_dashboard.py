...
def _summary(view: dict[str, Any], funding: dict[str, Any],
    ...
    fuel = bank + reserve
    # Источник числа — часть числа. Здесь это РСС: и потребность на завершение,
    # и остатки лимитов, и резерв 2.8/2.9 читаются с листа «Расчет стоимости
    # строительства», по итоговой строке глав 2 и 3. Прежде фраза говорила «по
    # утверждённому ДДС» — источник назывался чужим именем.
    #
    # Оговорку «что вне глав 2–3 сюда не входит» пришлось убрать: она указывала
    # не туда (владелец, 30.08.2026: «при чём тут вне глав. Потребности глав
    # 2–3 другие!»). Расхождение живёт ВНУТРИ этих глав — в одной книге по ним
    # два разных остатка: колонка «Средства на завершение» и «утверждённый
    # минус оплачено». Поэтому фраза называет не границу глав, а другой остаток
    # и разницу с ним.
    budget_need = float(funding.get("approved_remaining") or 0.0)
    money = (
        f"Деньги. По РСС на завершение глав 2–3 нужно {_fmt_money(need)}. "
        f"Доступно {_fmt_money(fuel)}: {_fmt_money(bank)} остатков лимитов "
        f"статей и {_fmt_money(reserve)} резерва 2.8/2.9 — "
        f"{fuel / need * 100:.0f}% этой потребности."
        if need > 0 else
        f"Деньги. Потребности на завершение по РСС (главы 2–3) не осталось; "
        f"доступно {_fmt_money(fuel)}.")
    if budget_need > 0 and abs(budget_need - need) > 1e6:
        money += (
            f" По утверждённому бюджету тех же глав остаток другой — "
            f"{_fmt_money(budget_need)} (утверждённый минус оплачено): в одной "
            f"книге по главам 2–3 два разных остатка, и какой из них "
            f"потребность, решает методика, а не расчёт.")
    out.append(money)
