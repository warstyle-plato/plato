...
def replace_function(name: str, body: str) -> None:
    ...
    text, count = re.subn(pattern, body.strip() + "\n\n", text, count=1)
    if count != 1:
        raise RuntimeError(f"Expected exactly one function {name}, got {count}")


if "0.12.15" not in text:
    raise RuntimeError("Expected v0.12.15 base")
text = text.replace("0.12.15", "0.12.16")
text = text.replace(
    'MANUAL_TEP_TEMPLATE_FILENAME = "PLATO_Шаблон_ТЭП.xlsx"',
    'MANUAL_TEP_TEMPLATE_FILENAME = "DevelopAid_Шаблон_ТЭП.xlsx"',
)
text = text.replace(
    "Нужен заполненный файл .xlsx из шаблона PLATO",
    "Нужен заполненный файл .xlsx из шаблона DevelopAid",
)

helper_marker = "def _telegram_dialog_data_lines(data: dict[str, Any]) -> list[str]:"
if "_TELEGRAM_PROJECT_CLASS_PRESETS" not in text:
    helpers = r'''
...
'''
    if helper_marker not in text:
        raise RuntimeError("Telegram dialog helper marker not found")
    text = text.replace(helper_marker, helpers.strip() + "\n\n\n" + helper_marker, 1)


replace_function(
    "_telegram_send_template",
    r'''
...
# Keep /plato as a hidden backward-compatible alias, but expose only /model.
text = text.replace('if command == "/plato":', 'if command in {"/model", "/plato"}:')
text = text.replace(
    '{"command": "plato", "description": "Открыть модель PLATO"}',
    '{"command": "model", "description": "Открыть модель DevelopAid"}',
)

# Remove the old product name from all Telegram-facing code while preserving
# the internal financial engine and its existing identifiers.
bot_start = text.index("_TELEGRAM_PUBLIC_BASE_URL")
bot_end = text.index("def _server_preset_meta", bot_start)
bot_section = text[bot_start:bot_end].replace("PLATO", "DevelopAid")
text = text[:bot_start] + bot_section + text[bot_end:]

# The Mini App sends the source label back to Telegram; keep that label clean too.
text = text.replace(
...
required = [
    "_TELEGRAM_PROJECT_CLASS_PRESETS",
    "flow_class_comfort",
    "flow_class_business",
    "flow_class_elite",
    "flow_class_custom",
    "Класс жилья / цены / себестоимость",
    "Версия: 0.12.16",
]
for marker in required:
    if marker not in text:
        raise RuntimeError("Missing marker after patch: " + marker)

# No old visible PLATO branding may remain inside the Telegram section.
bot_start = text.index("_TELEGRAM_PUBLIC_BASE_URL")
bot_end = text.index("def _server_preset_meta", bot_start)
if "PLATO" in text[bot_start:bot_end]:
    raise RuntimeError("Old PLATO branding remains in Telegram section")

path.write_text(text, encoding="utf-8")
print("DevelopAid v0.12.16 patch applied")
