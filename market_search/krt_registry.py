...
class KrtRegistry:
    ...
    def tender_orders(self, *, refresh: bool = False, max_pages: int = 12) -> dict[str, Any]:
        ...
        payload = {
            ...
            #
            # Прежний текст говорил «привязать распоряжение к площадке нечем» —
            # верно на день, когда он писался, и неверно с тех пор, как адрес
            # достаётся распознаванием скана: на проде 09.09.2026 адрес прочитан
            # у 27 распоряжений из 55, а к площадкам привязано 25. Оговорка «мы
            # этого не умеем» устаревает молча и читается как ответ источника.
            "note": ("Адрес есть только в скане распоряжения — ни в заголовке, "
                     "ни в карточке документа его нет; читаем распознаванием, и "
                     "непрочитанное названо. Распоряжение — документ города о "
                     "будущем аукционе, а не лот: лот на площадке ищется "
                     "отдельно, по торгам."),
        }
