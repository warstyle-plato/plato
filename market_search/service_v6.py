    ...
    def build_report(
        ...
        *,
        ...
        if subject.project_id is None and subject.subject_type != "krt":
            ...
            # эвристика подменяла площадку таким ЖК и его собственный прайс
            # уезжал в «цену окружения» (в каталоге получались миллионы ₽/м²).
        ...
        notes["findings"] = findings(
            ...
            # Комнатность, оплата и покупатели живут в блоках, а не в метриках
            # объекта: вывод по ним складывается из тех же чисел, что нарисованы
            # на экране, — второй счёт однажды разошёлся бы с картинкой.
            blocks=blocks,
        )
