...
Из песочницы `bnmap.pro` и `api.bnmap.pro` закрыты сетевой политикой —
соединение не устанавливается вовсе (403 на CONNECT), как у torgi.gov.ru,
bankrot.fedresurs.ru и НСПД. Живого ответа bnMAP у меня нет ни одного.

Значит **разбора здесь нет**: ни имён полей, ни адресов API, ни формы
оболочки. Есть проба, которая ходит с ядра и ПОКАЗЫВАЕТ ответ.
...
CANDIDATES: tuple[tuple[str, str], ...] = (
    ("корень", f"{BASE}/"),
    ("страница входа", f"{BASE}/login"),
    ("корень API", "https://api.bnmap.pro/"),
)
...
def probe(certs_dir: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "parsing": (
            "разбора нет намеренно: живого ответа bnMAP не видел никто, "
