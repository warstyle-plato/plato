...
# Ведущая группа — от одной цифры, групп по три — до трёх. Прежний шаблон требовал
# минимум двух цифр в первой группе, поэтому «3 306 021 ₽/м²» он читал с середины,
# как 306 021: элитная московская цена молча делилась на десять и всё равно
# попадала в допустимый диапазон, то есть выглядела правдоподобной.
_NUM = r"\d{1,3}(?:[\s\u00a0\u202f]\d{3}){1,3}|\d{5,8}|\d{1,4}(?:[.,]\d{1,2})?"
_MONEY = r"(?:₽|руб\.?|р\.)"
# Метр пишут четырьмя способами, и «за м²» встречается чаще, чем «/м²». Прежний
# шаблон знал только косую черту, поэтому из пяти ходовых форм цены читал одну.
