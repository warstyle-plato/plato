  ...
  on('#bngo', 'click', async function(){{
    ...
    try{{
      ...
      $('#bnout').innerHTML=verdictCard(data)+findingsCard(data)
        ...
        // Лестница цены по формату — то, ради чего этот источник и брали:
        // цену метра по комнатности «Пульс» не отдаёт вовсе, а без неё
        // вопрос «дешевеет ли метр с ростом лота» задать нечем.
        +'<div class="card"><h2>Лестница цены по формату</h2>'
          +roomsChart(market, data.deal_rooms||{{}})
          +'<div class="muted" style="font-size:12.5px;margin-top:6px">Цена метра'
          +' по комнатности, прайс на дату среза. У рынка метр дешевеет с ростом лота:'
          +' покупатель приходит с бюджетом, и за доступность малого формата платят'
          +' премией к метру. Плоская или растущая линия означает, что лестницы нет.'
          +' Пунктир — цена в СДЕЛКАХ этого проекта за последний год: разрыв с его'
          +' прайсом и есть скидка, которую дают по формату.</div></div>'
        +blocks
        ...
        +deepCard(data)
        // Цена В СДЕЛКАХ по месяцам — то, чего у «Пульса» нет вовсе: он даёт
        // прайс, то есть витрину. Рисует тот же `trendChart`; ряды помечены
        // как готовые разрезы, поэтому полосы квартилей нет — она была бы
        // полосой проекта из самого себя.
        +((data.deal_series||[]).length
          ? '<div class="card"><h2>Цена в сделках по месяцам</h2>'
            +trendChart(data.deal_series)
            +'<div class="muted" style="font-size:12.5px;margin-top:6px">Линия на'
            +' комнатность, ₽/м² по зарегистрированным ДДУ. Месяц без сделок такой'
            +' комнатности в ряд не входит: ноль здесь значит «сделок не было», а не'
            +' цену. Прайс этого же проекта — на графике выше.</div></div>'
          : '')
