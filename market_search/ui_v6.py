...
from .assessment import ADDRESS_PATHS
...
# Адрес в поле — не пример, а объект оценки. Зашитый «Москва, ул. Мишина, 46»
# выглядел ровно как подставленный из проекта: нажать «Найти аналоги» и получить
# рынок чужого района можно было не заметив. Поле пустое, пока адреса нет, и
# заполняется из тех же вводных, откуда его берут Платон и бот, — список путей
# подставляется из движка модуля, чтобы копии, которую негде обновить, не было.
_OLD_ADDRESS_INPUT = (
    '<label>Адрес<input id="mdAddress" value="Москва, ул. Мишина, 46" autocomplete="street-address"></label>'
)
_NEW_ADDRESS_INPUT = (
    '<label>Адрес<input id="mdAddress" placeholder="Москва, ул. Мишина, 46" '
    'autocomplete="street-address"></label>'
)

_OLD_RUN_HEAD = """async function runMarketDiscovery(){
  const status=document.getElementById('mdStatus');
  const result=document.getElementById('mdResult');
"""
_NEW_RUN_HEAD = """const MD_ADDRESS_PATHS=__MARKET_ADDRESS_PATHS__;
function mdModelAddress(){
  const source=(typeof inputs==='object'&&inputs)?inputs:{};
  for(const path of MD_ADDRESS_PATHS){
    let node=source;
    for(const key of path){node=(node&&typeof node==='object')?node[key]:null;if(node==null)break}
    if(typeof node==='string'&&node.trim())return node.trim().replace(/\s+/g,' ');
  }
  const lookup=source._land_lookup;
  const parcels=(lookup&&(lookup.parcels||lookup.items))||[];
  for(const parcel of parcels){
    if(parcel&&typeof parcel.address==='string'&&parcel.address.trim())
      return parcel.address.trim().replace(/\s+/g,' ');
  }
  return '';
}
function mdFillAddressFromModel(){
  const field=document.getElementById('mdAddress');
  if(!field||field.value.trim())return;
  field.value=mdModelAddress();
}
if(typeof openTab==='function'){
  const mdPreviousOpenTab=openTab;
  window.openTab=function(name,button){
    const opened=mdPreviousOpenTab.apply(this,arguments);
    if(name==='marketDiscovery')mdFillAddressFromModel();
    return opened;
  };
}
async function runMarketDiscovery(){
  const status=document.getElementById('mdStatus');
  const result=document.getElementById('mdResult');
  mdFillAddressFromModel();
  if(!document.getElementById('mdAddress').value.trim()){
    status.textContent='Укажите адрес площадки: в модели его пока нет, а искать вокруг примера бессмысленно.';
...
def _address_comes_from_the_project(page: str) -> str:
    for old, new in ((_OLD_ADDRESS_INPUT, _NEW_ADDRESS_INPUT), (_OLD_RUN_HEAD, _NEW_RUN_HEAD)):
        if old not in page:
            raise RuntimeError(
                f"Панель рынка изменилась, адресная подстановка не найдена: {old[:60]}"
            )
        page = page.replace(old, new, 1)
    return page.replace("__MARKET_ADDRESS_PATHS__", json.dumps(ADDRESS_PATHS, ensure_ascii=False), 1)
...
def install(core: Any) -> None:
    ...
    page = _address_comes_from_the_project(page)
