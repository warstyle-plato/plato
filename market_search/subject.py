    ...
    *,
    ...
    if find_krt:
        ...
        if territory:
            ...
            # Откуда площадка — часть ответа: у половины списка карточки в
            # каталоге нет вовсе, и её опознал опубликованный проект решения.
            # Подпись «взята из krt.mos.ru» была бы там неверной.
            origin = " ".join(str(territory.get("source") or "krt.mos.ru").split())
            notes = [
                f"КРТ взята из {origin}; официальная геометрия границ не получена."
            ]
