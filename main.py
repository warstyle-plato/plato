...
app = FastAPI(title="PLATO Development Investment Model", version="0.6.8")
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.6.8 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
  ...
  <div class="content">
    <div id="inputs" class="panel active">
      <div class="note" style="margin-top:0;margin-bottom:18px">Класс проекта выбирается в верхней шапке: <b>Комфорт / Бизнес / Элитный</b>. Пресет меняет стартовые цены квартир и коммерции, цену м/м и себестоимость надземной/подземной части.</div>
...
function renderRateCurveChart(){
 ...
 const W=1000,H=290,pL=50,pR=20,pT=20,pB=35;
 ...
 for(let v=min;v<=max;v+=2){
   grid+=`<line x1="${pL}" y1="${y(v)}" x2="${W-pR}" y2="${y(v)}" stroke="#e5e5e5"/><text x="5" y="${y(v)+4}" font-size="11" fill="#777">${v}%</text>`;
 }
 const goalX=x(Math.min(horizon,show.length-1));
 target.innerHTML=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">
  ${grid}
  <line x1="${goalX}" y1="${pT}" x2="${goalX}" y2="${H-pB}" stroke="#aaa" stroke-dasharray="5 5"/>
  <text x="${Math.max(pL,goalX-58)}" y="${pT+12}" font-size="10" fill="#777">горизонт ${horizon} мес.</text>
  <polyline points="${pts('high')}" fill="none" stroke="#777" stroke-width="2.4" vector-effect="non-scaling-stroke"/>
  <polyline points="${pts('base')}" fill="none" stroke="#050505" stroke-width="3.2" vector-effect="non-scaling-stroke"/>
  <polyline points="${pts('low')}" fill="none" stroke="#aaa" stroke-width="2.4" vector-effect="non-scaling-stroke"/>
 </svg>
 <div class="legend"><span><i style="background:#777"></i>Консервативная → ${inputs.rate_target_high_pct}%</span><span><i></i>Базовая → ${inputs.rate_target_base_pct}%</span><span><i class="gray"></i>Оптимистичная → ${inputs.rate_target_low_pct}%</span></div>`;
}
...
function loadLocal(){try{const x=JSON.parse(localStorage.getItem('plato_v04'));if(x){inputs=x.inputs||inputs;tep=x.tep||tep;rates=[];scenarioSelect.value=x.scenario||'base'}}catch(e){}}
function resetAll(){
