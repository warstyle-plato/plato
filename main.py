...
app = FastAPI(title="PLATO Development Investment Model", version="0.8.7")
...
def _llcr_from_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """LLCR = NPV of future CFADS over remaining PF life / PF principal at peak debt."""
    if not rows:
...
def calculate(req: CalcRequest) -> dict:
    ...
    return {
        ...
        "notes": {
            "llcr": "LLCR = NPV будущего CFADS за оставшийся срок ПФ / основной долг ПФ на дату пикового долга. Продажи до РВЭ учитываются через будущий релиз эскроу; выборки, проценты и погашения не входят в CFADS.",
...
<div class="shell">
  ...
  <div class="header">
    <div class="title"><h1>Девелоперская инвестиционная модель</h1><p>v0.8.7 · ТЭП · экономика · БРИДЖ · проектное финансирование · эскроу · LLCR</p></div>
    <div class="actions">
    ...
    <div id="finance" class="panel">
      <div class="card">
        <div class="llcr-hero">
          ...
          <div class="llcr-label">LLCR = NPV будущего CFADS за оставшийся срок ПФ / основной долг ПФ на дату пикового долга. Для многоочередного проекта контрольным является минимум по очередям; портфельный показатель показывается отдельно.</div>
        </div>
      </div>
