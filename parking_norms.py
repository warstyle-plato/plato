...
# Верхний край, когда коэффициентов нет (решение владельца, 06.09.2026: «к1 и
# к2 без выгрузки — ставь по верхнему краю с оговоркой»). К1 и К2 только
# СНИЖАЮТ потребность, поэтому единица даёт МАКСИМУМ мест — самый строгий
# ответ, а не «примерно норматив». Отказ был честнее по форме и бесполезнее по
# делу: у площадки без выгрузки не оставалось ни одного места, и в модели она
# выглядела как проект без парковки вовсе. Число обязано быть названо НАШИМ
# допущением, а не нормой города: это разные утверждения, и на экране они
# выглядят одинаково.
MOSCOW_K_UPPER_EDGE = 1.0

# Функции нет в приложении 1 — по сноске 6 число мест определяется заданием на
# проектирование, то есть норматива города здесь нет вовсе. Ставим по логике
# (владелец, 06.09.2026: «если норм нет надо как-то из логики ставить») и
# берём ставку ОБЫЧНОГО нежилого того же приложения: 63 м² ННП на место — она
# стоит и у делового управления (4.1), и у магазинов (4.4), то есть у двух
# самых частых нежилых функций. Это НАША логика, а не строка акта, и оговорка
# говорит это прямо. У Московской области такая развилка закрыта своим
# документом — п. 5.12 даёт правило 1 место на 50 м²; в Москве такого общего
# правила нет, поэтому и оговорка другая.
MOSCOW_FALLBACK_X2 = 63.0
MOSCOW_FALLBACK_SOURCE = (
    "ставка обычного нежилого приложения 1 (деловое управление 4.1 и магазины "
    "4.4) — 63 кв. м нежилой наземной площади на место; собственная логика "
    "DevelopAid, а не строка акта для этой функции")
...
# офиса и ТЦ не переносятся. Складывать их между собой тоже нельзя — 1080-ПП
# от 01.09.2026 сказал это прямо: «на 15% или на 10% (процент снижения не
# суммируется)». Московские коэффициенты сюда не переносятся тем более.
#
...
MO_RULES: tuple[dict[str, Any], ...] = (
    ...
    # приложение № 10 к Нормативам в редакции 1480/45 от 29.12.2021, страницы
    ...
    {"function": "fitness",
     ...
     "source": ("713/30, приложение № 10 в редакции 1480/45 от 29.12.2021: "
...
# А это — текст 1080-ПП от 01.09.2026 дословно, и он подтверждён. Абзац
# двадцать седьмой п. 5.12 переиздан: число осталось прежним (1 место на 50
# кв. м общей площади), а состав помещений расширился — к помещениям без
# конкретной функции и встроенно-пристроенным первым этажам добавлены
# многоуровневые паркинги независимо от функции. Прежняя редакция — 774-ПП от
# 02.07.2026, и в ней многоуровневых паркингов в этом правиле не было.
MO_FALLBACK_DENOMINATOR = 50.0
MO_FALLBACK_UNIT = UNIT_TOTAL_AREA_SQM
MO_FALLBACK_SOURCE = (
    "713/30, п. 5.12 в редакции 1080-ПП от 01.09.2026: помещения в нежилых "
    "зданиях без конкретного функционального назначения, встроенно-пристроенные "
    "нежилые помещения на первых этажах жилой застройки и многоуровневые "
    "паркинги независимо от функции — 1 место на 50 кв. м общей площади таких "
    "помещений; приложение № 10 применяется к остальным помещениям, кроме "
...
def moscow_required(
    ...
    *,
    ...
    if not x2_meta.get("x2"):
        # Отказ здесь был бы верен по форме и бесполезен по делу: у объекта
        # не оставалось ни одного места, и модель показывала проект без
        # парковки вовсе. Ставим по логике и называем её своей.
        x2_assumed = True
        assumptions.append(
            (x2_meta.get("reason") or "функции нет в приложении 1")
            + f"; ставим по логике — ставка обычного нежилого того же приложения, "
              f"{MOSCOW_FALLBACK_X2:g} м² ННП на место. Это собственная логика "
              "DevelopAid, а не норматив города")
        x2_meta = {
            "x2": MOSCOW_FALLBACK_X2, "vri": "", "label": "Функция вне приложения 1",
            "unit": UNIT_ABOVE_NONRES_SQM, "source": MOSCOW_FALLBACK_SOURCE,
    ...
    # одна: подписанный нормой, он читается как расчёт города, — поэтому
    # оговорка идёт вместе с числом, а не рядом в другом месте.
    ...
    if not x2_meta["source_confirmed"] and not x2_assumed:
        # У принятой по логике ставки «не сверен по тексту акта» звучит как
        # претензия к акту, которого нет: она уже названа нашей строкой выше.
    ...
    return {
        ...
        "k1": k1_value,
        "k2": k2_value,
        # Чьё это число — часть самого числа: принятое нами и пришедшее из
