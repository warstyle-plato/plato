...
# Как считает штатный калькулятор ГлавАПУ встроенно-пристроенные помещения.
# Это НЕ строка акта: она восстановлена по выгрузке и воспроизводит ответ
# города до единицы (77:05:0004001, 22.08.2026 — 15 мест). Строки для
# встроенных помещений в приложении 1 нет вовсе, а «60» из акта даёт на тех же
# метрах 25. Пока не доказано обратное, ответ города весит больше нашего
# прочтения таблицы, написанной для отдельно стоящих объектов.
MOSCOW_BUILT_IN_X2 = 100.0
MOSCOW_BUILT_IN_SOURCE = (
    "воспроизводит штатный калькулятор ГлавАПУ (выгрузка 77:05:0004001, "
    "22.08.2026); строки для встроенных помещений в приложении 1 нет")

# Приложение 6: К1 — доступность рельсового каркаса по ПЕШЕХОДНЫМ путям до
...
def moscow_x2(function: str, at: str = "") -> dict[str, Any]:
    ...
    if not meta:
        return {"x2": None, "reason": f"функция «{function}» не заведена в справочнике Москвы"}
    vri = meta["vri"]
    ...
    return {"x2": None, "vri": vri,
            "reason": f"строки {vri} нет ни в одной известной редакции приложения 1"}
    ...
    *,
    ...
    if built_in:
        x2_meta = {
            ...
            "unit": UNIT_SPP_SQM, "source": MOSCOW_BUILT_IN_SOURCE,
            "source_confirmed": False, "edition": "glavapu_observed", "note": "",
        }
        assumptions.append(
            "встроенные помещения считаются по строке, воспроизводящей штатный "
            "калькулятор ГлавАПУ, а не по таблице приложения 1")
    else:
