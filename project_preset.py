...
# Копия здесь ВЫНУЖДЕННАЯ: движок импортирует этот модуль, значит обратной
# дороги нет и `TEP_DEFAULT` отсюда не спросить. Цена копии уже заплачена —
# список отстал: ФОК завели четвёртым отдельно стоящим объектом (05.09.2026),
# сюда он не попал, и очередь, объявившая `sports` рядом с именем, отдавала
# движку только офисы, а вложенной формой не отдавала НИЧЕГО. Ровно то, о чём
# написано абзацем выше, только вторым заходом: метры молча не доезжали, а
# подпись очереди выглядела верной.
#
