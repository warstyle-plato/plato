...
# Корни сертификатов монтируются и стенду: без них `api.krt.mos.ru` отвечает
# CERTIFICATE_VERIFY_FAILED, и на странице участков КРТ площадка «не опознана»
# — на рабочем ядре это давно смонтировано, а стенд повторял ошибку, которой у
# прода нет. Каталог только на чтение, как у выкатки прода.
