...
def _replace_once(text: str, old: str, new: str, label: str) -> str:
    if new in text:
        return text
    if old not in text:
        raise RuntimeError(f"DevelopAid hotfix: marker not found: {label}")
...
'''
    if "def _telegram_send_help(" not in text:
        if handler_marker not in text:
            raise RuntimeError("DevelopAid hotfix: Telegram handler marker not found")
...
'''
    if "function closeTelegramWebAppAfterResult()" not in text:
        if send_marker not in text:
            raise RuntimeError("DevelopAid hotfix: sendTelegramResult marker not found")
...
try:
    _main = _find_main_file()
    if _main is not None:
        _patch_main(_main)
except Exception as exc:
    print(f"DevelopAid runtime hotfix failed: {exc}", file=sys.stderr)
